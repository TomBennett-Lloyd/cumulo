...
- **Merge policy** (`.claude/workflow.json`): a PR merges autonomously only if every changed file is Markdown and CI is green. Any other diff gets the `awaiting-review` label and waits for the user — keep working on independent tickets meanwhile. Review feedback is logged in `docs/review-feedback.md`; gate changes are proposed via retro PR and decided by the user.
- **Plan approval** (`.claude/workflow.json`): while `planApproval.mode` is `required`, `/plan-issue` stops after posting the plan and waits for user approval before `/execute`.
