...
export default defineConfig({
  test: {
     ...
     * import: this app already depends on that package, but nothing in its
     * module graph depends on the guard, and a `setupFiles` entry keeps it that
     * way. `src/aws-test-guard.test.ts` is this app's committed proof that the
     * wiring above actually bites — `main.ts` builds a real document client at
     * module scope, and `main.test.ts` imports it.
     */
