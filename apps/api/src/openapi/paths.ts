...
export const apiPaths: PathsObject = {
  ...
  '/v1/fleet/forecast': {
    get: {
      ...
      summary: 'The fleet’s summed forecast, from now forward',
      description: [
        'The whole fleet’s forecast from the current time forward, **already summed**: one',
        'point per hour carrying the fleet total, its uncertainty band, how many sites',
        'reported that hour and how much nameplate capacity stood behind them. The sum is',
        'the same on every load, so the forecast service computes each location’s',
        'contribution as it produces it and this route reads them back in one query.',
        'Points already past belong to `GET /v1/sites/{siteId}/series` and its explicit',
        'bounds; `GET /v1/fleet/actuals` is the same fleet read the other way, and still',
        'returns raw readings. **An empty fleet, and a fleet with nothing forecast yet,',
        'are both a 200 with an empty `points` array**: a fleet with no forecast is a fact',
        'about the forecast schedule rather than about whether the fleet exists, which is',
        'the distinction `GET /v1/sites/{siteId}/forecast` draws for one site. The',
        'response is all or nothing: a fleet total that could not be assembled in full is',
        'a 500 rather than a total quietly short a site. Carries the Open-Meteo',
        'attribution that must be displayed with the data; like every endpoint here it',
        'makes no upstream weather calls. Rate-limited per address.',
      ].join(' '),
