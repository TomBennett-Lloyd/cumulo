...
const componentSources = [
  ...
  },
  {
    ...
    description: [
      'The whole fleet’s forecast over one forward horizon, already summed: one entry per',
      'hour carrying the fleet total, its uncertainty band, how many sites reported that',
      'hour and how much nameplate capacity stood behind them. The last of those is the',
      'divisor a percent-of-capacity view needs, which is why it is on the wire rather',
      'than left to a client that no longer receives the rows to derive it from. An empty',
      '`points` array is a normal 200 for `SiteForecastResponse`’s reason applied to a',
      'whole fleet: the hours appear when the next forecast cycle writes them. Carries the',
      'Open-Meteo attribution that must be displayed alongside them.',
    ].join(' '),
  },
  {
    name: 'FleetSite',
