 ...
 * **this app's vitest wiring**: the guard being correct in `@cumulo/storage`
 * says nothing about whether this config still points at it. This app needs the
 * proof more than most — `main.ts` builds a real document client at module
 * scope, and `main.test.ts` imports that module.
 *
 ...
 * loopback evidence is asserted one level down.
 */
describe('the AWS test guard, as this app wires it', () => {
  ...
  it(
    ...
    async () => {
      ...
      try {
        // A table that exists nowhere, read through the adapter this app serves
        // `GET /v1/sites/{siteId}` with. If this ever answers, or fails as
        ...
        await expect(sites.getFleetSite('cumulo-aws-test-guard-probe')).rejects.toMatchObject({
          name: 'StorageError',
