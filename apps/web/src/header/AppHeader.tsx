...
export interface AppHeaderProps {
  ...
  /** The listing, plus anything created this session. */
 ...
 * the same `selectedSiteId` the markers read, and both of those live in the
 * dashboard — so either the bar moves down to the state or the state moves up to
 * the bar. Moving the state up would put the fleet listing, the selection and
 * the first-forecast poll in the shell purely so a text input could see them,
 * which is the larger of the two changes by a distance.
 *
 ...
 * (`apps/web/src/App.test.tsx` pins that as an assertion, and the boundary still
 * discharges the Open-Meteo credit itself).
 *
 * Layout is `apps/web/src/app.css`'s `.app-header` — one wrapping flex row.
 *
 ...
 * line, the row used to wrap and the bar became two lines tall — all of that
 * second line height the map does not get, which is the same weighing
 * `HeaderMenu.tsx` states and the reason that button exists at all. So at those
 * widths the field is replaced by an icon of its own and opens a full-width bar
 * directly under the row, which is one line tall while it is up and no lines
 * tall the rest of the time.
 *
 ...
 * for it, and an effect runs after that gesture is over. `AppHeader.test.tsx`
 ...
 * is open under it, deliberately unlike the menu's dialog-then-popover pair:
 * there, two surfaces sat one inside the other and each had content of its own
 * to go back to. Here a two-step Escape would make a reader press twice to leave
 * a surface one press opened, to land back on a field that is about to disappear
 * anyway.
 *
...
export const AppHeader = ({
  ...
  const handleBarKeyDown = (event: KeyboardEvent<HTMLDivElement>): void => {
    /*
     * The guard `SiteSearch.tsx` documents at the top of its own keydown
     * handler, owed twice because this is a second listener on one press: the
     * field's keydown bubbles up here, so an Escape abandoning a composition
     * arrives after the search has correctly ignored it and would take the whole
     * bar away from a reader who was mid-word.
     */
  ...
  return (
    <header className="app-header">
      ...
      >
        {/* `aria-hidden` for `Brand.tsx`'s reason: a mark that says nothing the
            button's name does not already say should not be in the accessibility
            tree twice. Weight and colour are `header.css`'s. */}
