 ...
 * transitive import, correctly, and `docs/standards/architecture.md` rule 1 wants the app→package
 * edge visible in the manifest rather than inherited by accident.
 *
 ...
 * fleet, and this one is not: `packages/shared/src/fleet.ts` places clusters across Ireland *and*
 ...
 *
 * Civil twilight: the sun 6° below the horizon, the conventional line past which there is
 ...
 * its own and can be tested on its own (`docs/standards/structure.md` rule 1).
 *
 ...
 * hour too late, visibly so at sunrise and sunset. `packages/forecast/src/physics-forecast.ts`'s
 ...
 * propagates to a `NaN` zenith, and `NaN > NIGHT_ZENITH_MIN_DEG` is `false` — so a garbled hour is
 * reported as day. That is the deliberate direction: this layer's whole contract is that absence
 * of the flag means "draw nothing" (see `ForecastChartPoint.night`), and a malformed hour is
 * exactly a case where drawing nothing is right. It shades no hour it cannot justify.
 */
 ...
 * once whether it has a fleet to answer about at all, then asks only the per-hour question. This is
 * not a closure factory in `docs/standards/structure.md` rule 2's sense (no object of functions, no
 * state a reader has to trace).
 *
