...
export interface InfoTipProps {
   ...
   * list or a paragraph is valid here, which is what the fleet chart's legend
   * needs (#429).
   */
 ...
 *
 * The page had sentences explaining things that were on screen, each taking a
 * line of vertical space from the surface it described on every render for every
 * reader (#265). This is where the surviving one went — reachable in one press,
 * and costing nothing while nobody is asking. The others went rather than moved:
 * no home at all is the right one for a description something else on the page
 * already gives.
 *
 ...
 * has no sample to speak and so cannot compete with it. So the change is
 * announced the way a disclosure announces it — the button carries
 * `aria-expanded`, which is state on the control the reader just pressed rather
 * than an interruption. (A `role="status"` mounted with its text already inside
 * it announces nothing anyway, #161, so the live-region version would have had to
 * keep an empty container on every page for every tip, which is the cost the
 * budget rule exists to refuse.)
 *
 ...
 * (`docs/standards/structure.md` rule 7). Ask the rule's question — if the menu
 * changed, would this be wrong until it changed the same way? For the dismissal
 * *policy*, yes: two overlays in one app dismissing on different gestures is an
 * inconsistency a reader can feel. For the code around it, no: the menu's
 * listener stands down while its modal dialog is open, and this has no modal to
 * stand down for. So the shared portion is smaller than the hook that would
 * carry it. The moment extraction becomes right is already written down:
 * `docs/tech-debt.md` holds a pending decision about a fourth route (dismissal
 * on focus leaving), and applying it means applying it to both, in one place.
 *
 ...
 * `apps/web/src/dashboard/fleet-panel.css`, whose `.fleet-chart-controls` rule
 * states the obligation where it is discharged.
 *
 ...
 * itself is what produced that: an icon button near the right edge of the page
 * is not a box the panel's measure can be clamped inside, so its width ran past
 * the viewport. The row is, which is why the anchor moved outwards rather than
 * the measure inwards. A new consumer that cannot offer a positioned ancestor is
 * a design question for that surface, not a case for a second rule here.
 *
...
export const InfoTip = ({ label, children }: InfoTipProps): ReactElement => {
  ...
  // rule 1), and so genuinely an effect. Only while open: a listener on the
  // document for every tip on the page at rest is a cost paid by readers who
  // never press one.
  useEffect(() => {
    if (!open) {
      return undefined;
