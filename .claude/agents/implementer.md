...
- **Verify before reporting.** Run the chunk's `Verify:` command(s), plus `pnpm verify`, before claiming DONE. Report actual output honestly — a failing check reported honestly is fine; a false DONE is the one unforgivable failure. During a parallel wave you share the worktree with sibling chunks: scope your evidence to your own files, and report a composite red or `git status` noise that lives entirely in a sibling's files as exactly that — the orchestrator re-runs the composite at the wave boundary; never fix sibling code.
