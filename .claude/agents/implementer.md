...
- **Verify before reporting.** Run the chunk's `Verify:` command(s), plus `pnpm lint` and `pnpm typecheck`, before claiming DONE. Report actual output honestly — a failing check reported honestly is fine; a false DONE is the one unforgivable failure.
