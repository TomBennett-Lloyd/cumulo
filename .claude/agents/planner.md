...
A plan that CHANGES a stated value or standing claim — a capacity, a billing mode, a transport, a mechanism — must run the reviewer's synonym-tolerant claim sweep at plan time and enumerate every restatement into the chunks' file lists, so the review loop verifies emptiness instead of discovering the list. #156's plan enumerated three files of stale-capacity comments from memory of where the claim lived; seven more restatements (README stack table, test docblocks, an adapter comment, iam.tf, two pool claims) surfaced serially across all three review cycles, and the one the third cycle caught was a cost claim the change itself had made false. The sweep is cheap at plan time and unaffordable at three findings a cycle.
