...
Spawn this agent with `run_in_background: false`. It is the one exception to the repo's background-sub-agent default: browser agents stall silently with no output, a spawner cannot `TaskStop` another agent's stall, and every orphaned run leaves a dev server holding its worktree's port. A synchronous run makes the stall visible to its owner immediately. Four consecutive background browser agents stalled during #17/#19 and cost two full cycles.

**Sessions from different worktrees run concurrently.** Each one serves its own tree on its own derived port (below), so there is no shared resource to queue on and no reason to wait for another lane's browser work to finish. The one serialisation rule that survives is per-worktree, and it is not about ports: **never measure a tree something is concurrently mutating.** Vite's HMR reloads the page under you, so a smoke against a worktree an implementer is still editing measures a moving target — that tree's chunk finishes first, then you measure. Two smokes against the _same_ worktree are the same problem twice and also serialise. (Before #459 every server bound 4173, so all of this queued globally; the queue was the port, not the work.)

## Server ownership

The dev server is yours from start to finish.

- **The port is derived, never chosen.** `apps/web/e2e/lane-ports.ts` hashes the worktree's root path into a lane: the dev server binds 25173–25684 in a worktree (5173 in the main checkout), the e2e preview server 24173–24684 (4173 in the main checkout). Read the number rather than guessing it — `pnpm -C <worktree> dev` prints the URL it bound, and `--strictPort` means it either binds that port or fails loudly, so a server that started is a server on the port you expect. Do not pass `--port`: a hand-picked number is exactly what the derivation replaced.
- Before starting: if the port is already held, find out **whose** it is before killing anything. `lsof -nP -iTCP:<port> -sTCP:LISTEN -t` gives the pid and `lsof -a -p <pid> -d cwd -Fn` gives its working directory. A holder inside your own worktree is a stray from an earlier orphaned run — kill it, and never reuse it. A holder inside a **different** worktree is the rare lane collision (two trees hashing together, ~1 in 512): that is somebody else's live session, so kill nothing, and return INCONCLUSIVE naming the colliding tree.
- `preview_start` with a **named** config resolves `.claude/launch.json` against the MAIN checkout whatever your cwd is, so a named start verifies `main`'s tree rather than the branch you were sent to check — and the worktree has no `launch.json` of its own to find. Verifying a worktree branch therefore means starting that worktree's dev server yourself: `pnpm -C <worktree> dev` via `Bash`, then attach with `preview_start({url})` on the URL it printed. Starting it that way means owning it — a backgrounded server you walk away from is the failure this section exists to prevent.
