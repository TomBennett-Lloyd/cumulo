...
5. Look for patterns: ≥3 similar entries, or the same phase hurting in ≥50% of recent tasks.
6. Each pattern → `gh issue create --label workflow` with the aggregated evidence and a proposed workflow change. Delete captured entries.
7. Leave singletons; delete entries stale enough that the workflow has since changed under them.
