...
## 2026-08-11 — issue #401 — restatement-batch-401

- **Category**: approved-no-changes
- **Feedback**: The owner is asked to ratify the member decisions this batch takes, and this entry states the ask only — it predicts no part of the answer. (a) Rule 9's ADR exemption narrowed, so that what an ADR holds immutable and exempt is its reasoning and its decision, while a stated value an ADR owns sits inside the restatement discipline like any other owned value, with its ledger living in that ADR's `## Amendments` section (#401). (b) The Amendments convention stated in the shapes practice already uses: the hybrid, the error-correction category, the obligation a quoter carries, and the re-check owed to characterisations derived from an amended value (#402). (c) The restatement ledger's trigger (when a value acquires one), its writer (whoever finds the second carrier), its floor-plus-sweep form, and the `feedbackLogLedger` key added to `.claude/workflow.json` — itself a `humanAlways` file (#403). (d) Rule 12 and the comment-borne-arithmetic convention (#405). (e) A standards-index row in `CLAUDE.md` routing rule 12's trigger, added beside the other `docs/standards/architecture.md` rows and worded to mirror that doc's own Trigger clause for the same act (#405). (f) This batch's `docs/adr/**` diff as a whole, that path being `humanAlways` too. What the owner decides goes in **Verdict** below, at merge.
- **Why**: `docs/adr/**`, `.claude/workflow.json` and `CLAUDE.md` are `humanAlways` paths, and this batch edits every one of them, so it cannot merge on green CI plus a review-loop APPROVE — the owner is the gate. The ask is put provisionally because `merge.humanAlwaysRule` orders this entry ahead of the `awaiting-review` label: the entry is part of the diff the owner is being asked to approve, and the CI `merge-ritual-gate` job fails a labelled PR whose diff does not touch this file, so the entry necessarily exists before the owner has seen the PR — the moment at which this file can honestly hold the question but not the answer. The form above is the one `## Entry format` declares for that state.
- **How applied**: Batch #401 — members #403 (the ledger's trigger, writer and form, and the workflow key), #402 (the Amendments convention, and ADR 0007's live misquotation patched), #401 (rule 9's narrowed exemption, the stale ADR→ADR citations annotated as-they-stood, ADR 0005's bound ledgered and ADR 0002's stale argument trued — the anchor member this entry rides), #405 (rule 12, its `CLAUDE.md` standards-index row, and `simulated-uncertainty.ts`'s counterfactual docblocks brought into compliance). At merge, whoever merges fills **Category** and **Verdict** on the branch before the label comes off.
- **Verdict**: Approved without changes — owner, in chat, 2026-08-11 ("427 looks good to go"), covering all six parts of the ask above: the four member decisions (a)–(d), the `CLAUDE.md` standards-index row (e) that was flagged for the one-read precisely because it is the batch's only line in the most-gated file, and the `docs/adr/**` diff as a whole (f). Filled on the branch by the merge owner before the label came off, so a reader can tell a checked approval from an unchecked prediction without reading git history. The category was assigned here, at verdict time, rather than predicted pre-label — which is what the provisional form exists to make visible.

## 2026-08-25 — PR #469 — retro: prose standard, review targeting, relay pattern

- **Category**: approved-no-changes
- **Feedback**: Ratify three retro decisions taken in chat the same day: `docs/standards/prose.md` (#464), the review-loop's targeted cycles and deferral-by-shape (#465), and the relay pattern written into `task-orchestrator.md` (#466). The one `CLAUDE.md` line is the standards-index row retargeting derived-figure prose to the new standard.
- **Why**: `CLAUDE.md` is a `humanAlways` path.
- **How applied**: One commit per issue on `464-prose-standard`; this entry rides #464's commit. Written in the register the standard asks for.
- **Verdict**: Approved without changes — owner, in chat, 2026-08-25 ("PR 469 is good to merge").

## 2026-09-08 — issue #476 — verify-tier-classifier

- **Category**: pending — filled at merge
- **Feedback**: The owner is asked to decide two things, and this entry states the ask only — it predicts no part of the answer. (a) The amended `CLAUDE.md` Commands bullet: `pnpm verify` is now tiered, the tier is derived mechanically from the change set by `.claude/scripts/verify-tier.sh`, hand-picking legs stays banned, and a newly added gate joins `verify:full` rather than `verify`. (b) One deliberate divergence from the issue's specification, flagged on [#476](https://github.com/TomBennett-Lloyd/cumulo/issues/476) before implementation. #476 specified the soundness guard as a blanket grep — any `\.md` reference anywhere in the test trees forces the full tier, "today it should be empty". It is not empty and cannot be: 501 hits across 129 of this repo's 207 test files, because `check-adr-index.test.sh` and `check-markdown-links.test.sh` build markdown fixtures by the dozen. Implemented literally, the guard would pin every run on the full tier and the ticket would deliver nothing. The guard shipped here is scoped to the change set instead: for each changed `.md`, its repo-relative path and its basename, grepped as fixed strings across every `*.test.*` / `*.spec.*` / `*.test.sh`; any hit is printed and forces full. What the owner decides goes in **Verdict** below, at merge.
- **Why**: `CLAUDE.md` is a `humanAlways` path, so this cannot merge on green CI plus a review-loop APPROVE — the owner is the gate. The ask is put provisionally because `merge.humanAlwaysRule` orders this entry ahead of the `awaiting-review` label, so it necessarily exists before the owner has seen the PR. The divergence is the half worth the one-read: it changes what the guard claims. The blanket form claims "no test in this repo can observe any markdown"; the shipped form claims "no test can observe the markdown in **this** change set" — narrower, and the one the tier actually depends on, but it is a narrowing the issue did not authorise, and it costs false positives on prose that merely names a file (a test comment reading "testing.md rule 7" forces a full run on any change to `testing.md`). That direction is the safe one, which is why it was taken rather than referred back mid-flight.
- **How applied**: `.claude/scripts/verify-tier.sh` plus its harness; `package.json` splits `verify` (root line, then the classifier) from `verify:full` (today's composite, verbatim); `.github/workflows/ci.yml` calls `verify:full` explicitly, so CI's coverage is unchanged by design. Evidence on the PR: both synthetic tier lines, `verify:full` green at 149 s against the docs tier's 8 s, and the blanket-grep census above. At merge, whoever merges fills **Category** and **Verdict** on the branch before the label comes off.
- **Verdict**: Approved without changes — owner, in chat, 2026-08-25 ("do the rest in sequence", after reviewing the tier design and the follow-on #477). Both parts of the ask: (a) the `CLAUDE.md` Commands bullet as amended; (b) the guard scoped to the changed files rather than a blanket grep — the blanket form is infeasible here (markdown-fixture harnesses), and the scoped form fails closed in the same direction. Filled on the branch by the merge owner before the label came off.
