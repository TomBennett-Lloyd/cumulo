...
- Triage note (2026-08-03): kept, and sharper than when written — the wording it protects is in **no** file. `grep` over `.claude/agents/reviewer.md` and `.claude/skills/review-loop/SKILL.md` finds neither "re-derive" nor "re-run what you doubt"; it exists only as ad-hoc orchestrator dispatch phrasing, so there is nothing to trim and nothing a cold sub-agent inherits. Below the ≥3 threshold at two catches (#167, #161), so not filed — but the action if a third lands is now "write it into `reviewer.md`", not "protect it".
