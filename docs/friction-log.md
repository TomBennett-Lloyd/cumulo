...
## 2026-09-11 — batch 3 of #467 and the #473 warmer lane (one dropped connection, three failure shapes)

- Phase: process
- Observed: the owner's wifi dropped twice; each time every running agent died (`ENOTFOUND` from the agents' own API, or a 600 s stall the watchdog reported as "no progress") and its in-flight work — five trimmed files, a set of Terraform fixes — sat uncommitted with no report. Recovery was two rounds of hand-reconstructing disk truth into fresh dispatch briefs. A third shape surfaced after: a background task entry ("rewrap header.css paragraphs") showing as running for 50 minutes with a 0-byte output file and no OS process — a command whose owner had died five minutes before the harness moved it to background. Diagnosis recipe: 0-byte task output + `ps` shows nothing under the worktree ⇒ ghost, safe to stop. What is outside the repo's control: the watchdog cannot distinguish network-down from stuck, and retry loops inside an agent do not survive the agent. What is inside it and now written into `.claude/agents/ticket-agent.md` rule 4/5(f) and `/run-issue`'s `RESUME`: commit every verified step, tick the plan comment, resume from disk and ticks rather than memory. The merge owner's own shell chains failed the same way (a poll died on `error connecting to api.github.com`; one chain mis-gated the label on CI rather than on the merge) — the fix for that class is the merge ritual as a script, filed as its own issue.

## 2026-09-11 — PR #512 (issue #497 batch 1)

- Phase: process
- Observed: agents inherit the dispatcher's cwd, and a failed `git worktree add` falls through silently to `checkout -b` in place. The lane was told to create `.claude/worktrees/497-sweep-1` and made all six of this PR's commits on branch `497-sweep-1` checked out in the top-level session's own worktree instead. Branch name, commit list and `git status` all read as correct from inside; the difference first surfaced when the merge owner's rebase chain `cd`'d into a lane path that did not exist and ran its unconditional tail — `pnpm verify`, then `git push` — in the session checkout. The work was correct and merged, and nothing was damaged, by luck rather than by any gate. Two things have no signal at all: the fall-through itself, and the distinction between "the worktree exists" (`git worktree list`) and "I am in it". The assertion now in `.claude/agents/ticket-agent.md` rule 1 and `.claude/agents/task-orchestrator.md` rule 1 covers the lane's own half; #513 owns the merge-owner half (`cd <dir> || exit 1`, and `merge-pr.sh` resolving the worktree from the branch rather than trusting a path argument). Corroboration from #514's own lane, a different mechanism with the same root: the harness pinned that agent's write access to the worktree its session launched in and refused edits to the worktree its dispatch had just created — a lane cannot assume where it is, whether the cwd came from inheritance or from the harness.
