...
## 2026-07-31 — No test exercises the surfaces the app actually mounts

- Where: `apps/web/src/dashboard/Dashboard.test.tsx` (passes a stand-in `mapRegion`), `apps/web/src/App.test.tsx` (forces the `#tokens` hash), `apps/web/src/map/MapView.tsx`, `apps/web/src/map/SiteMarkers.tsx`
- What: every committed test of the dashboard substitutes the map region, and every test of the shell renders the token preview instead of the dashboard — so the composition the app boots into (`App` → `Dashboard` → `MapRegion` → `MapView` + `SiteMarkers`) is asserted by nothing. Each substitution is individually justified (jsdom has no WebGL, and `testing.md` rule 3 rules out mocking maplibre to get around it), which is what makes this systemic rather than a missed test: the suite is honest about what it covers and the _sum_ still leaves the shipping surface unproven. This branch is the evidence, not the hypothesis — three defects lived exactly in that gap and were all found by hand in a browser: marker clicks propagating into the add-site flow, maplibre's marker element being stamped with a duplicate interactive role, and a canvas that never resized to its container. Same root cause as `testing.md` rule 7's warning, one level up: not a knob turned off per-test, but a whole layer that no configuration under test ever includes. The Playwright-harness ticket already carried out of #17's plan is where this belongs, and these three defects are its motivating evidence.
