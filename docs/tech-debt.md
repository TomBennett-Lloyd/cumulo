...
- What: the plot states both facts in chrome, and `role="img"` collapses the whole SVG subtree to one `aria-label` — so a reader in focus mode hears `06:00 — 0.9 Actual` with no zone and no unit, from numbers that are UTC kilowatts. Not a regression: the table twin discharges the treatment's obligation today, and `docs/design/chart-treatment.md` names the twin's heading a valid home for the clock. But "every chart states the clock" now has a spoken surface that does not, and the readout is the surface a keyboard reader actually lands on. The real fix is a `chart-copy.ts`-owned suffix for the spoken string plus a treatment amendment naming the readout as a surface the clock has to reach — a decision that binds every future chart readout, which is why it is here rather than a line in this PR (#20, #16)
- Source: #106 review cycle 1
