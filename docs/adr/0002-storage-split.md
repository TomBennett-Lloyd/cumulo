...
Provisioned capacity can throttle, and the counter-argument deserves its weight. Throttling surfaces as `ProvisionedThroughputExceededException`, which the AWS SDK's DynamoDB clients retry automatically with exponential backoff and full jitter — pinned at 2 attempts, 1,000 ms throttling base delay (amended by #122 from the default 4; see Amendments). The 2026 retry set requires `AWS_NEW_RETRIES_2026=true`, so #13 must pin it explicitly rather than inherit whatever the runtime supplies, per `docs/standards/error-handling.md` rule 3: timeout, retry count, and backoff are visible at the call site, not implicit in library defaults.
...
5. **Pin the SDK retry behaviour explicitly.** The full-jitter 2026 retry set is gated behind `AWS_NEW_RETRIES_2026=true`; the client sets it rather than inheriting it, per error-handling rule 3 — currently 2 attempts, 1,000 ms throttling base (amended by #122 from the default 4; see Amendments).
...
- **2026-08-09 — `cumulo-series` provisioned 14 WCU / 21 RCU → on-demand (#258).** Revisit trigger 8's first clause, fired live and then escalated: the `WriteThrottleEvents` alarm this document's Consequence 2 requires went to ALARM on `cumulo-series` with ~650 throttle events across two colliding cycles, and 12 of 24 forecast messages failed on first delivery (2026-08-05, minutes after #156 removed the upstream pacing that had been masking it); the owner escalated the recurring alarm mail as noise on 2026-08-09. Nothing was lost — SQS redelivered every message, the last on its third receive — but what absorbed the cliff was redelivery patience rather than capacity. The cause is the granularity #156 named, arriving on the other side of the queue and larger: one queue message is one `putForecasts` call carrying every site at that location across the whole horizon — **~240 write units**, drained as ~10 sequential `BatchWriteItem` pages of 25, with two such messages in flight at once — which outruns 14 WCU/s and the bounded retry budget above it however low the 7% per-cycle utilisation is. The response is the one trigger 8 names for this exact table — flip `billing_mode`, one attribute, no migration, no key change. **What this costs the document, stated plainly: no table draws on the Region's 25 WCU / 25 RCU any more, so "Total against the free tier: 14 WCU / 21 RCU of 25 / 25" and the hybrid capacity mode it summarises now describe no deployed resource**, and the storage-allowance question under "One thing we could not confirm" stops being moot, because every table holding data is on-demand — `infra/README.md`'s storage cost table prices that exposure. The standing rule survives with the caveat both flips earned: batch-shaped load is necessary for provisioning and, on two out of two attempts, not sufficient, because the rule sizes sustained rate and what broke was burst shape. The values now live on the `cumulo-series` table in `infra/storage/tables.tf`.
- **2026-08-10 — per-dashboard-load read sizing ≈ 27 → ≈ 52 read units (#264; recorded by #334).** Not a value this document set moving under it, but the workload it priced changing shape: #264 gave the fleet a simulated-actuals series beside its forecasts, so a dashboard load has run two fleet-wide `series` fan-outs ever since — ~25 read units each, plus the ~2-unit `FLEET` Query on `sites`, ≈ 52 a load — and #296 later moved both fan-outs behind the API's two fleet routes, which moved where the units are spent and not how many. Every read-side figure derived from ≈ 27 halves where it still describes anything: ~50 sustained loads/minute would be ~25, the ~250 burst-absorbed loads ~125. The body is deliberately not trued up inline, and the reason is the guardrail. **Seven** in-body sites carry a figure derived from ≈ 27, and these are all seven rather than the ones a reader happened to hit: "The read side fits too", which states the ≈ 27 arithmetic itself; the burst-reserve correction immediately after it ("An earlier draft of this ADR read that as…"), the paragraph where ~50 dashboard loads per minute sustained, the `21 × 300 = 6,300` reserve and the ~250 loads absorbed instantly are derived in the first place; the "Sustained dashboard loads/min at 21 RCU" column of the fleet-headroom table, every one of whose 252 / 101 / 50 / 25 / 13 halves; the ~250-loads-of-burst figure under "The failure mode, stated honestly"; option E's rejection, which restates that same ~25-per-load arithmetic and the ~50-sustained / ~250-absorbed correction that dissolves E's central objection; the #17 polling consequence's ~25 read units per poll per tab, whose conclusion — polling cadence, not viewer count — #264 sharpens rather than unsettles; and "Assumed scale, restated"'s ~27 per load with ~25 of it provisioned. Two grounds keep an amendment off these, and they overlap rather than partition: each is at once reasoning the capacity-mode choice was carried by, which an amendment never touches, or arithmetic off the 21 RCU the 2026-08-09 entry above already records as describing no deployed resource, and most are both. Either ground alone is sufficient, so no site needs sorting into one — a trued-up ≈ 52 would misstate what this decision was sized against while describing nothing deployed either. **Every per-dashboard-load read figure in the body above therefore states pre-#264 sizing and should be read as such** (figures on other read paths — `weather`'s hindcast, the ~2 units on `sites` — are untouched by #264 and stand); the current per-load figure lives in the `series` table's comment block in `infra/storage/tables.tf`, whose file-header restatement ledger names every site that carries it (as architecture rule 9 stood when this entry was written it exempted ADRs from the ledger wholesale, and that blanket exemption is the reason this entry gave for ownership moving out of this document rather than a ledger moving in beside it; #401 narrowed it on 2026-08-11 — an ADR's decision and reasoning stay exempt, while a stated value an ADR owns sits inside the ledger discipline with its ledger in that ADR's `## Amendments` section — see `docs/standards/architecture.md` rule 9 as it now stands, and the annotation entry below. The disposition is unmoved under the narrower rule: the per-dashboard-load read figure is a value the code owns rather than one this ADR owns, so no ledger was owed inside this document for it before the narrowing or after).
- **2026-08-11 — the 2026-08-10 (#264) entry's citation of architecture rule 9 marked as its pre-#401 wording (#401).** No stated value moves here. That entry closed by citing rule 9's ADR exemption in the present tense and arguing from it: the exemption was given as the reason ownership of the per-dashboard-load read figure moves to `infra/storage/tables.tf` rather than a restatement ledger moving in beside it here. #401 narrowed the exemption on 2026-08-11 — an ADR's decision and reasoning stay immutable and exempt, but a stated value an ADR owns is amendable and sits inside the ledger discipline, its ledger living in that ADR's `## Amendments` section — which left the citation standing as a live argument for the opposite of the rule, the carrier `docs/standards/architecture.md` rule 11 gives to the change that moves the claim. The citation now carries the as-it-stood annotation `docs/adr/README.md` blesses, marking its vintage and pointing at rule 9 as it now stands, plus a clause stating that the disposition survives the narrowing: the per-dashboard-load read figure is a value the code owns, never one this ADR owned, so no ledger was owed inside this document for it under either wording. Nothing else in this document changes — the #264 entry's disposition and its wholesale historical declaration stand, no body figure or reasoning is touched, the capacity table keeps the provisioned row the 2026-08-09 (#258) entry deliberately left standing, and this ADR remains `accepted`. **Quoter sweep — a floor, not a census.** `command grep -E` over the repo's `.md`, `.ts` and `.tf` files (excluding `node_modules`), run 2026-08-11, on three arms, with each file's lines joined before matching (`tr '\n' ' '`) — because a sentence that wraps across two comment lines is invisible to a line-scoped grep, and the line-scoped form of arm 1 missed `infra/storage/tables.tf`, whose header ledger carried the retired claim across exactly such a break. The first is the citation's own family, generalised past the wording this entry used so that a differently phrased copy still matches — `exempts? ADRs?|ADRs? [^.]{0,40}(immutable and )?exempt|exempt from the (restatement )?ledger` — and it is stated in that generalised form deliberately: an arm quoting the retired sentence literally would match this entry and every later report of it, so the family arm is both the wider search and the one that stays readable. The second is a paraphrase arm for the same claim worded without those verbs, `ADRs?[^.]{0,80}(outside|not subject to|do(es)? not carry|never carr)[^.]{0,60}(ledger|discipline)`. The third is a bare `rule 9` arm over every citation of the rule in the repo. What the first arm establishes outside this document is neither a list of its hits nor a property of them, and the joining is why: lines are joined per file before matching, so a hit is a whole file, while the thing that can be read and adjudicated is a passage — and one file can hold a passage naming the retired claim beside another leaning on it. What is recorded is therefore the reading, passage by passage, of what this entry adjudicated: `docs/tech-debt.md`'s captured-entry row names the claim as history, preserving the titles of the original tech-debt entries — the historical claim rule 9's past-tense clause covers; `docs/friction-log.md`'s triage note and `docs/review-feedback.md`'s entry for the batch name it as this batch's subject; and `infra/storage/tables.tf`'s header ledger names it as the thing being retired, now stating the narrowed rule and the blanket exemption only as what it dropped. Whatever else those files hold is unadjudicated by this sweep rather than shown clean. The second arm returned nothing anywhere, this document included. The third returns the rule's citations across the repo, too many to characterise one at a time; what intersecting it with arm 1 buys is a bound and not a verdict — a citation arguing from the ADR exemption has to mention the exemption, so the intersection is contained in arm 1 by construction and puts no file in front of a reader that arm 1 had not already surfaced. A carrier that neither names rule 9 nor uses those words is unsearched by this sweep rather than shown absent.

- **2026-09-11 — A5's fleet fan-out superseded in part by ADR 0009, and the 2026-08-10 (#264) entry's per-load figure marked as-it-stood (#494).** No value this document states moves here, and the decision this entry touches is not amended — it is **superseded in part**, which is why ADR 0009 exists rather than a third figure appearing in this section. `### Fleet-wide aggregation (A5)` chose fan-out and named the condition that reopens it (revisit trigger 4: the aggregate endpoint becoming hot enough that fan-out latency or cost is visible). The condition was met on latency and measured — 1,753.9 ms p50 / 2,998.9 ms p95 warm on the fleet **forecast** route — so 0009 replaces that route's fan-out with a precomputed per-location roll-up summed at read, and this ADR's index row carries the partial-supersession annotation `docs/adr/README.md` blesses. **A5 stands for the fleet actuals route**, which still fans out until 0009's fast-follow ticket lands, and nothing else in this document is touched: the table split, the key design, the capacity mode, the TTL posture and every option this ADR considered stand as written.

  What is amended is one quotation's vintage. The 2026-08-10 (#264) entry above states the then-current per-dashboard-load read figure (≈ 52, ~25 for each of two fleet fan-outs plus ~2 on `sites`) in the present tense and argues from it. #494 replaced the forecast half of that figure with one Query of the `#FLEET` partition — ~18 eventually-consistent read units for a 48-hour window at the canonical fleet — so the load is ≈ 45, and its forecast half no longer grows with the site count at all. **That entry's ≈ 52 should therefore be read as stating the figure as it stood between #264 and #494**; its wholesale historical declaration over this document's _body_ figures is unaffected and stands, as does its disposition that the per-load figure is a value the code owns rather than one this ADR owns. The current figure still lives in the `series` table's comment block in `infra/storage/tables.tf`, whose header ledger names every carrier and now names ADR 0009 as one of them.

  **Quoter sweep — a floor, not a census.** Run 2026-09-11, `command grep -rn` over `docs/`, `infra/`, `apps/` and `packages/` (excluding `node_modules`), on three arms: `2,880|2880` for the per-cycle write count, `1\.48|≈ 52|read units? (a|per) load|per-dashboard-load` for the read and cost figures, and — for the forecast log census #494 also moves by one line — a **family** arm, `(ten|[0-9]+) billed lines|[0-9]+ lines per invocation|8[0-9] MB`, rather than the two literal spellings the first pass used. The literal form is recorded here because it is the instructive part: it missed `infra/README.md`'s forecast log-cost row (`10 billed lines/invocation`) and `infra/forecast/outputs.tf`'s `That ~88 MB` back-reference, both of which the generalised arm returns, and both of which the review pass on this branch caught. An arm keyed to the phrasing the amender happened to use is not a sweep of the claim (architecture rule 10). Every hit was read and trued up in the same commit where it is mutable: `infra/storage/tables.tf`'s `series` section (the owner) and its header ledger, `infra/README.md`'s storage `series` cost row, its three-fleet-sizes note, its schedule-running total, its ingestion "forgotten stack" note and teardown paragraph, its forecast DynamoDB driver row, its forecast log-cost row and its forecast log census, `infra/forecast/outputs.tf`'s log and DynamoDB census entries, `infra/forecast/event-source.tf`'s concurrency argument, `infra/ingestion/outputs.tf`'s driver line, the `listSites` docblock in `apps/web/src/data/fleet-data-source.ts`, and the fleet-vs-poll comment in `apps/web/src/data/use-first-forecast.test.tsx`. The `POLL_INTERVAL_MS` docblock in `apps/web/src/data/use-first-forecast.ts` carries the ratio and no unit figure since #467, and is owed nothing. `apps/ingestion/README.md`'s 2,880 is the Open-Meteo daily-quota table and names a different quantity entirely — a false positive on the literal, sorted out by reading. The one immutable carrier is this document, handled above. A carrier phrased without any of those literals is unsearched by this sweep rather than shown absent.
