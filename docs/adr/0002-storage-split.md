...
- **2026-09-11 — A5's fleet fan-out superseded in part by ADR 0009, and the 2026-08-10 (#264) entry's per-load figure marked as-it-stood (#494).** No value this document states moves here, and the decision this entry touches is not amended — it is **superseded in part**, which is why ADR 0009 exists rather than a third figure appearing in this section. `### Fleet-wide aggregation (A5)` chose fan-out and named the condition that reopens it (revisit trigger 4: the aggregate endpoint becoming hot enough that fan-out latency or cost is visible). The condition was met on latency and measured — 1,753.9 ms p50 / 2,998.9 ms p95 warm on the fleet **forecast** route — so 0009 replaces that route's fan-out with a precomputed per-location roll-up summed at read, and this ADR's index row carries the partial-supersession annotation `docs/adr/README.md` blesses. **A5 stands for the fleet actuals route**, which still fans out until 0009's fast-follow ticket lands, and nothing else in this document is touched: the table split, the key design, the capacity mode, the TTL posture and every option this ADR considered stand as written.
  ...
  **Quoter sweep — a floor, not a census.** Run 2026-09-11, `command grep -rn` over `docs/`, `infra/`, `apps/` and `packages/` (excluding `node_modules`), on three arms: `2,880|2880` for the per-cycle write count, `1\.48|≈ 52|read units? (a|per) load|per-dashboard-load` for the read and cost figures, and `10 lines per invocation|ten billed lines` for the forecast log census #494 also moves by one line. Every hit was read and trued up in the same commit where it is mutable: `infra/storage/tables.tf`'s `series` section (the owner) and its header ledger, `infra/README.md`'s storage `series` cost row, its three-fleet-sizes note, its schedule-running total, its ingestion "forgotten stack" note and teardown paragraph, its forecast DynamoDB driver row and its forecast log census, `infra/forecast/outputs.tf`'s log and DynamoDB census entries, `infra/forecast/event-source.tf`'s concurrency argument, `infra/ingestion/outputs.tf`'s driver line, the `listSites` docblock in `apps/web/src/data/fleet-data-source.ts`, and the fleet-vs-poll comment in `apps/web/src/data/use-first-forecast.test.tsx`. The `POLL_INTERVAL_MS` docblock in `apps/web/src/data/use-first-forecast.ts` carries the ratio and no unit figure since #467, and is owed nothing. `apps/ingestion/README.md`'s 2,880 is the Open-Meteo daily-quota table and names a different quantity entirely — a false positive on the literal, sorted out by reading. The one immutable carrier is this document, handled above. A carrier phrased without any of those literals is unsearched by this sweep rather than shown absent.
