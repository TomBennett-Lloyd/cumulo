...
- **Sites:** 60 + 40 = **100** — exactly ADR 0002's fleet-headroom row **as it stood** when the cap was chosen (14% of the free 25 WCU, a 693-second ingest drain with zero burst assumed, 25 sustained dashboard loads/minute at 21 RCU; since overtaken: #258 moved `cumulo-series` off provisioned capacity, #264 re-based the per-load read — 0002's Amendments). The cap was chosen to land on a row that document had already costed rather than on a round number.
...
- **2026-08-11 — this document's citations of ADR 0002's provisioned capacity marked as-it-stood (#401).** This document quotes that capacity in order to reason from it, and 0002 has since amended the figures it quotes: **`cumulo-series` 21 provisioned RCU → on-demand** (0002's Amendments, 2026-08-09, #258), owned now by the `cumulo-series` table in `infra/storage/tables.tf`; and the **per-dashboard-load read sizing**, which #264 re-based on 2026-08-10 — 0002's own entry states by how much — and which is what the fleet-headroom row's sustained-loads-per-minute column is computed from, owned now by that table's comment block. §1's "Which routes are limited" picks the limited routes by weighing a maximum-width series read against the provisioned RCU, and §2's `**Sites:**` bullet picks the cap of **40** by landing the worst-case fleet exactly on that fleet-headroom row. Each is a premise — the route list and the number 40 were argued from the figures as they then stood — so per `docs/adr/README.md` § Amendments each carries an as-it-stood annotation and neither piece of reasoning is reworded. The cap stays **40**, owned by `MAX_USER_SITES` in `packages/shared/src/site.ts`; re-picking it against the amended row would be revisit trigger 4's work, not an amendment's. The sites named here are **a floor rather than a census** — what a sweep of this file found, `command grep -nE '([0-9,]+ ?(RCU|WCU)|provisioned|6,300|read units|fleet-headroom)'`, run 2026-08-11.
