...
2. **CloudFront appearing in front of the API** — #144's distribution or #21's custom domain. That is the moment WAF becomes attachable, and it is the documented upgrade path from trigger 1: re-cost $5.00/ACL + $1.00/rule + $0.60/M against the traffic actually seen, and move rate limiting to the edge where it costs no invocations.
