...
- **2026-08-11 — the `cumulo-series` capacity this document reasons against marked as-it-stood, and the "Worst case, sustained" bound given its ledger (#401).** Two halves, and neither moves a figure.

  _The annotation._ Consequences' "The cost guard is not a capacity guard" paragraph quotes ADR 0002's `cumulo-series` sizing to argue from it — **21 provisioned RCU → on-demand** (0002's Amendments, 2026-08-09, #258), the value now owned by the `cumulo-series` table in `infra/storage/tables.tf` — and derives the 300-second 6,300-unit burst reserve and the read-throttling failure mode from it. That is a premise, not a carrier: the gap this document chose to make visible is the gap as it then was, so per `docs/adr/README.md` § Amendments the paragraph now carries an as-it-stood annotation and its arithmetic and conclusion stand untouched. Where the same traffic bills now is already stated by the 2026-08-10 scope entry above. Elsewhere in the body, 0002's provisioned capacity is cited in the present tense — the Context's "Four DynamoDB tables sit inside the always-free provisioned allowance (0002)" — and **every such statement in the body above describes pre-#156/#258 sizing and should be read as such**, which is the inline half owed to it.

  _The restatement ledger for the "Worst case, sustained" bound_ (architecture rule 9: the bound is a stated value this ADR owns, so its ledger lives here). Known carriers, **a floor rather than a census**:

  - `infra/README.md` — `## Cost` → `### API stack`, which asserts the bound and its per-term breakdown; the same file also leans on it outside that section, in the api-stack overview ("an arithmetic bound of ≈ $36/month") and in the Cost preamble's log-sizing rule ("that same figure feeds the ≈ $36 worst case below").
  - `infra/api/gateway.tf` — the cost-guard comment above `default_route_settings`, which derives the bound from the throttle it is guarding.
  - `infra/api/outputs.tf` — the stack's what-it-costs header, in its log-sizing note and again in its closing worst-case line.
  - `infra/api/alarms.tf` — the header's argument that the budget alarm at 50% of ~$100 "would never fire at all" under the bound: an inference from it, which moves when it moves.
  - `apps/api/README.md` — the abuse-layer table's stage-throttle row, which cites this ADR and the bound together.
  - ADR 0006 — a **quoting ADR**, so not trued up but annotated and entered in its own Amendments: it computes its worst-case table on top of this bound, and its 2026-08-10 entry already carries the amended figure.

  Every mutable carrier above still states the pre-amendment ≈ $36; the `infra/README.md` one is the mixed-basis case the 2026-08-10 re-basing entry describes, its gateway term still on the us-east-1 rate while its CloudWatch term is already eu-west-1. Propagating the amended bound to them is #376's work — this entry names them, and edits none. The sweep behind the list is claim-shaped rather than keyed to the old literal (rule 10), run 2026-08-11 over the repo's Markdown, TypeScript, Terraform and shell sources with `node_modules` excluded:

  ```sh
  # the bound's own figures
  command grep -rnE '\$(36|38\.94|39)([^0-9]|$)|≈ ?\$3[0-9]'
  # its derivations, which carry no copy of the figure
  command grep -rniE '(worst[- ]case|arithmetic bound|third of the (~?\$100|ceiling))'
  ```
