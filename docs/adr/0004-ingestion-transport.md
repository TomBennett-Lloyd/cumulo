...
- **Read-back erodes ADR 0002's `weather` read sizing** — not arithmetically, but at the level of the reasoning. 0002 allocated `weather` 3 RCU (**as it then did**; `cumulo-weather` moved to on-demand — 0002's Amendments, #156) on an explicit premise: "its only read paths are offline … and #12's forecast service, which receives weather on the stream rather than reading it back. Nothing on `weather`'s read path sits in front of a user." Read-back makes that false. The added volume is modest — twelve Queries of ~15 KB each is roughly 24 read units per cycle — but it puts a read on the then-3-RCU table inside #17's ~60-second add-a-site path, which is exactly the class of claim 0002 sized against. Changing a premise another ADR provisioned capacity on, in order to save $0 against an option that also costs $0, is a bad trade at any price.
...
- **2026-08-11 — this document's citation of ADR 0002's `weather` capacity marked as-it-stood (#401).** Option E's read-back rejection quotes that capacity in order to reason from it: `weather` **3 RCU → on-demand** (0002's Amendments, 2026-08-03, #156), the value now owned by the `cumulo-weather` table in `infra/storage/tables.tf`. The figure is a premise here, not a carrier — option E was rejected _because_ a read-back path would sit a user in front of a table sized for offline reads only — so per `docs/adr/README.md` § Amendments it is annotated rather than trued up: the quotation now reads "allocated … (**as it then did**)" and the table it names "the then-3-RCU table", and the rejection argument is untouched. Elsewhere in the body, 0002's provisioned capacity is cited in the present tense — the Context's "`weather` gets 3 RCU partly because …" and the Consequences' "four DynamoDB tables inside the permanently free 19 WCU / 24 RCU and 25 GB" — and **every such statement in the body above states pre-#156/#258 sizing and should be read as such**, which is the inline half owed to those sites and the reason no per-figure annotation is added on top of it. That list of citing sites is **a floor rather than a census**: it is what a sweep of this file found, `command grep -nE '([0-9]+ ?(RCU|WCU)|-RCU|read sizing|provisioned)'`, run 2026-08-11.
