...
- Basemap tiles and styles by [OpenFreeMap](https://openfreemap.org/), rendering map data © [OpenStreetMap contributors](https://www.openstreetmap.org/copyright) ([ODbL](https://opendatacommons.org/licenses/odbl/)). Both credits are also shown in the app itself, overlaid on the map's bottom edge — the tile credit and the Open-Meteo credit are separate obligations and neither substitutes for the other.
- The interface is set in [Inter](https://rsms.me/inter/) by Rasmus Andersson, under the [SIL Open Font License 1.1](https://openfontlicense.org/) — the licence text ships beside the font in [`packages/ui/src/tokens/fonts/`](packages/ui/src/tokens/fonts/). It is **self-hosted**, as a latin-subset variable woff2 covering the 400–600 weight span the app uses: no font CDN is contacted at runtime, which is both a privacy property and what the app's `font-src 'self'` content security policy requires. Shipping the face rather than borrowing the reader's system font is what makes this repo's measured-pixel derivations claims about the artefact instead of about the machine that measured them (see [#463](https://github.com/TomBennett-Lloyd/cumulo/issues/463)).
