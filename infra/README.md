...
- **The `series` row at all three fleet sizes, since the row itself only carries two.** The arithmetic is `series items per cycle × 730 × $0.705/M`, and a site contributes `48 × 2 = 96` of them plus one generation actual once #264's producer runs, while #494's roll-up adds a term that scales with **locations** rather than with sites — `12 × 48 = 576` partials a cycle at the canonical twelve: **~3,456/cycle ≈ 2.52 M units ≈ $1.78/mo** (the canonical fleet — ~30 sites over the 12 locations, forecast items plus partials, and the only one of the three that is currently real), **5,426 ≈ 3.96 M ≈ $2.79/mo** (ADR 0002's ~50-site envelope, actuals included, which the rest of this table is sized against), **10,276 ≈ 7.50 M ≈ $5.29/mo** (ADR 0006's 100-site cap, the number to hold against the ceiling). It is the platform's largest single line by an order of magnitude and still 5% of the ceiling at a fleet four times the size the demo has ever run — which is the whole reason the flip needed no `on_demand_throughput` ceiling, for the same argument the weather note below makes.
