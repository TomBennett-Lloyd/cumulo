...
That decision has a consequence worth stating plainly: `terraform output` prints values that embed the account id. Do not paste raw output into committed files, PR bodies, or issue comments — quote the shape (`arn:aws:iam::<account-id>:role/cumulo-github-actions`), not the digits.
...
Teardown is a project requirement, so it is stated rather than assumed. This stack has no ordering trap at all: its state lives in a bucket another stack owns, nothing here is another stack's foundation, and — unlike the Kinesis stream ADR 0004 replaced — every resource in it is free whether it is destroyed or forgotten. (A _left-enabled schedule_ is the caveat: it spends in the storage stack, not here — see "Whether to leave it up".)
...
Expect `7 to destroy`. It completes on a **non-empty** bucket — `force_destroy = true` in `s3.tf`: every object in this one is a build artefact a `pnpm --filter @cumulo/web build` reproduces. Budget **5–15 minutes**: CloudFront disables the distribution, waits for that to propagate to every edge, and only then deletes it.
...
| Resource group                                                                     | Billing basis                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Estimate                                       |
| ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **On-demand capacity — `series`** (the forecast fan-out's writes)                  | **Metered, no free allowance** since #258: $0.705/M write request units. Each cycle turns every location's horizon into forecast items — ~2,880/cycle at the canonical fleet, so `2,880 × 730` ≈ **2.10 M write units/month**. Reads bill at $0.1415/M: a dashboard load spends ~50 units on this table — ~25 covering every site's partition for the fleet's forecasts and ~25 again for its simulated actuals — plus ~2 over the `FLEET` partition on `sites`, so ≈ 52 a load, thousands of loads to a cent. The per-load arithmetic is owned by the `series` section of `storage/tables.tf` (#264 doubled it; ADR 0002's ≈ 27 is amended). **$0 whenever the schedule is not running** — driven by the forecast stack's consumer (its DynamoDB row points here) and the api's reads (the api stack's own row for its visitor-driven `series` reads is pending — #243; its DynamoDB row today costs #473's warmer reads on `sites` and nothing on this table) | **≈ $1.48/mo** (≈ $4.99 at #29's 100-site cap) |
...
| **Total, schedule running**                                                        | The standing total plus both capacity activity rows, at the canonical fleet — `series` ≈ $1.48 and `weather` ≈ $0.30                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **≈ $1.78/mo**                                 |
...
- **The `series` row at all three fleet sizes, since the row itself only carries two.** The arithmetic is `forecast items per cycle × 730 × $0.705/M`, and a site contributes `48 × 2 = 96` of them plus one generation actual once #264's producer runs: **~2,880/cycle ≈ 2.10 M units ≈ $1.48/mo** (the canonical fleet — ~30 sites over the 12 locations, forecast items only, and the only one of the three that is currently real), **4,850 ≈ 3.54 M ≈ $2.50/mo** (ADR 0002's ~50-site envelope, actuals included, which the rest of this table is sized against), **9,700 ≈ 7.08 M ≈ $4.99/mo** (ADR 0006's 100-site cap, the number to hold against the ceiling). It is the platform's largest single line by an order of magnitude and still 5% of the ceiling at a fleet four times the size the demo has ever run — which is the whole reason the flip needed no `on_demand_throughput` ceiling, for the same argument the weather note below makes.
...
| **DynamoDB writes** (`cumulo-series` via BatchWriteItem)           | ~2,880 items/hour → **~2.10 M write units/month**, metered on-demand since #258. This stack adds load _and_ a line — the line lives in the **storage** cost table's `series` row, because storage owns the resource                                  | **$0.00/mo here; drives ≈ $1.48/mo under storage** |
| **CloudWatch logs — stored bytes** (30-day retention on one group) | **Bills for existing**: ~$0.03/GB-month retained. **10 billed lines/invocation × ~8,760 invocations × 1 KB ≈ 88 MB/month** (census below), so ~$0.0026/mo at list — inside the account's free **5 GB** of storage and the free **5 GB** of ingestion | **$0.00/mo**                                       |
...
- **The log-group size above is a bound, not a measurement — and the census is written out so it can be checked against the source.** An invocation emits `forecast.message.outcome` and `forecast.batch.summary` (`apps/forecast/src/handler.ts`), one of each because `batch_size = 1` in `infra/forecast/event-source.tf` makes a batch a single record; one `forecast.actuals.outcome` (`apps/forecast/src/simulate-actuals.ts`) per site of the location that record speaks for, which is five in the canonical fleet; and Lambda adds its own `START`, `END` and `REPORT`. That is **10 lines per invocation**, ~87,600 a month, priced the way ingestion's are — 1 KB per line, rounded up from the ~250 bytes per _invocation_ that ADR 0005's ~6.5 GB/month at 25.92M requests works out at. So **10 × 8,760 × 1 KB ≈ 88 MB/month**: the platform's largest log line by volume, and still under 2% of the account's free 5 GB. Note which way `batch_size` moves it — at a batch of _n_ the count is _n_ × (1 + sites at its location) + 4 lines per invocation rather than 10_n_, so raising it lowers the bytes per message. And note which term the fleet moves: the per-site half is `sites × 730` a month whatever the fleet's shape, so the sensitivity is to **locations**, not to sites. Spreading the same fleet over more locations buys more invocations and therefore more of the five fixed lines each — co-location is a log-volume decision as well as an Open-Meteo-quota one.
