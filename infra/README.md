...
- **A month is 730 hours** (8,760 ÷ 12), which is what AWS's own pricing pages mean by "per month". Every clock-driven volume below is that times its rate: an hourly cycle is **~730 invocations/month**, twelve messages an hour is **~8,760/month**.
- **A log group's size is a census times a line size**, and the census includes Lambda's own `START`, `END` and `REPORT` — CloudWatch bills those exactly like an application line, and omitting them is how a log estimate ends up several times under. Where a group carries application JSON lines whose size nobody has measured, they are priced at a deliberately generous **1 KB per line** and the result is labelled a ceiling rather than an estimate. Where the only lines are Lambda's own three — the api stack, which logs nothing on a healthy request — ADR 0005's **measured ~250 bytes per invocation** is used instead: there is no unmeasured application line to cushion against, and that same figure feeds the ≈ $36 worst case below, which an arbitrary 4× cushion would misstate rather than bound.
...
| **Lambda compute**                                                 | 256 MB × even a full 300 s cycle is ~56,000 GB-seconds/month, ~14% of the always-free **400,000 GB-seconds**; a real cycle is a fraction of it                                                                                                    | **$0.00/mo** |
...
| **CloudWatch logs — stored bytes** (30-day retention on one group)         | **Bills for existing**: ~$0.03/GB-month retained. A healthy request logs **no application line at all** (this service logs only failures), so the census is **3 billed lines/invocation** — Lambda's `START`/`END`/`REPORT`, which is precisely what ADR 0005's ~250 B/invocation measures. **~10,000 invocations × ~250 B ≈ 2.5 MB/month**, ~$0.00008/mo at list — inside the account's free **5 GB** of storage and the free **5 GB** of ingestion | **$0.00/mo** |
