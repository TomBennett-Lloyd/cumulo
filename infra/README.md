...
- **The throttle is the bound.** Delete `default_route_settings` from `infra/api/gateway.tf` and the worst case stops being finite — but only half of that is silent now. `throttling_rate_limit` is a declared infra mirror (#133), held strictly above the web fan-out's launch rate, so removing it — or the block around it — exits 2 at `pnpm verify` rather than passing. Deleting `throttling_burst_limit` on its own still goes unnoticed, and nothing else in the platform would notice either until the bootstrap stack's budget alarm fired at 50% of $100. Those two numbers belong in review whenever this stack changes.
