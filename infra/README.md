...
Then wait out two ticks (about ten minutes) and ask the log group how many execution environments answered. Each environment writes its own log stream, so counting distinct streams counts containers:
...
# expect over 15 minutes: invocations 6 (3 ticks × 2 targets), containers 2, coldStarts 0 once the first two ticks have run
```

`containers 1` means the two deliveries were serialised rather than dispatched together — the residual `warmer.tf` names. One tick landing that way is expected occasionally; every tick landing that way is a finding.
...
aws cloudwatch get-metric-statistics --namespace AWS/Events --metric-name Invocations \
  ...
  --period 3600 --statistics Sum --query 'Datapoints[].Sum'
# expect: 24 over a full hour — 12 ticks × 2 targets
```
