...
| **CloudWatch logs — stored bytes** (30-day retention on one group) | **Bills for existing**: ~$0.03/GB-month retained. ~13 JSON lines/hour ≈ ~9,400 lines and under ~10 MB/month, so ~$0.0003/mo at list — inside the free **5 GB** of storage (a separate **5 GB** covers ingesting them) | **$0.00/mo** |
...
- **A forgotten stack is all but free, and it is not inert.** Unlike every other resource in the platform, this one _does things_ while nobody is looking: an enabled schedule spends ~288 Open-Meteo calls/day against the 10,000/day free tier and grows an unconsumed queue. That is a quota and hygiene concern, not a billing one. The one thing it does keep spending on is the log group, and `retention_in_days = 30` is what makes that a rolling ~10 MB rather than an unbounded pile — see the teardown section above.
- **The log-group size above is a bound, not a measurement.** ADR 0005 costs ~6.5 GB/month of API log ingestion at 25.92M requests, which implies ~250 bytes per logged record; rounding that up to 1 KB a line and multiplying by the line rate is what produces the "under 10 MB/month" figure. It is an over-estimate on purpose: the honest claim is a ceiling, not a meter reading.
...
| **CloudWatch logs — stored bytes** (30-day retention on one group)         | **Bills for existing**: ~$0.03/GB-month retained. ~2.5 MB/month at demo volume (~250 bytes/record × ~10,000), ~$0.00008/mo at list — inside the free **5 GB** of storage and the free **5 GB** of ingestion | **$0.00/mo** |
...
| **CloudWatch logs — stored bytes** (30-day retention on one group) | **Bills for existing**: ~$0.03/GB-month retained. One line per message, ~8,760 a month — under ~9 MB/month, so ~$0.0003/mo at list, inside the free **5 GB** of storage and the free **5 GB** of ingestion | **$0.00/mo** |
...
**Standing cost is a fraction of a cent**, and the honest version of that is worth a sentence rather than a rounding. S3 charges for stored bytes whether or not anybody asks for them, and it is charging for 1.7 MB. The ingestion, api and forecast stacks each have an at-rest line too — their log groups — but those sit inside an always-free 5 GB pool, so they are $0.00 as billed; this one is not, because S3's free storage tier is a 12-month allowance this document deliberately does not count. That is the real difference, and it is why this is the only stack in the platform whose standing total is written as ≈ $0.01/mo rather than $0.00/mo. Everything else here is free while idle, and `terraform destroy` takes even the fraction to zero.
