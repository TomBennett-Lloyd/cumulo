...
`api_origin` needs no entry either, and leaving it out is a real answer rather than an omission: it defaults to empty, which is the demo-mode deployment whose build makes no API calls and whose Content-Security-Policy therefore admits no API origin. If the demo is to show real data, derive the value — never retype it — and set it in `web.auto.tfvars`:
