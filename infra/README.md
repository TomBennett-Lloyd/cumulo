...
**The same query is how the fix is judged over time.** Re-run it with a `--start-time` a month back and `by ispresent(@initDuration)` instead: before #473 every visit produced a cold row, and afterwards `coldStarts` should be the deploys and the occasional environment recycle, not the visitors.
...
Three conventions hold across every table below (the third has one recorded gap: the api stack's missing DynamoDB row, [#243](https://github.com/TomBennett-Lloyd/cumulo/issues/243)). They are stated once here rather than restated per stack, because the previous edition of this document quietly used two different ones and the tables disagreed with each other:
...
**What #473 changed is not the total but the meaning of "idle".** A forgotten api stack is no longer inert: the warmer's rule fires twice every five minutes whether or not anybody is looking, which is the property the ingestion stack used to hold alone. It buys nothing back in this table — every row it touches is still $0 — and it costs ≈ $0.005/month on the storage stack's meter, half a cent to keep the demo's first paint warm. The lever if that is ever unwanted is the rule's own `state`: disabling it takes the last two rows to nothing without moving a resource.
