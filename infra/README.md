...
| `api`       | `infra/api/`       | The fleet API Lambda and the API Gateway HTTP API of [ADR 0005](../docs/adr/0005-fleet-api-hosting.md), including the stage throttle that bounds its bill, two alarms, the warm-start schedule of [#473](https://github.com/TomBennett-Lloyd/cumulo/issues/473), and the CI deploy grant for it. |
...
| `web`       | `infra/web/`       | The demo SPA's private S3 origin bucket, the CloudFront distribution in front of it, the origin access control it reaches the bucket through and the response headers policy it serves every response with, and the CI deploy grant for content sync and cache invalidation. No alarms.          |
...
There is a second coupling between those two stacks that no dependency graph shows: `infra/forecast/lambda.tf`'s function `timeout` and `infra/ingestion/transport.tf`'s `visibility_timeout_seconds` are the two halves of ADR 0004's 6× floor, and raising the first without the second is a correctness bug. Both files carry the obligation in a comment, and the `check:infra-mirrors` gate in `verify` holds it mechanically: the floor is a declared pair with Terraform on both sides, checked at its declared factor, so a one-sided edit fails CI rather than waiting for someone to remember the comment.
...
`web` is the newest, and it is the first stack that serves a human directly: a private S3 bucket holding `apps/web`'s production build, a CloudFront distribution reaching it through an origin access control, and the deploy grant CI uses to sync content and invalidate the cache. Its state lives in `bootstrap`'s bucket and its grant attaches to `bootstrap`'s role from `infra/web/deploy.tf` (convention 8), which is the whole of its coupling to anything — **there is no cross-stack reference to `infra/api` in either direction**, and every string that makes the demo work travels between them through the operator rather than through Terraform. The API's URL now travels here twice, from the one `api_endpoint` output and never retyped from each other: into the SPA at _build_ time as the `VITE_API_BASE_URL` repo variable, so the app knows where to call, and into this stack at _apply_ time as `api_origin` in `infra/web`'s gitignored tfvars, so the response headers policy's Content-Security-Policy `connect-src` lets the browser make the call at all. Travelling the other way, the distribution's domain reaches the API as `web_origins` in `infra/api`'s gitignored tfvars, so its origin check admits the demo's writes. Three operator-published strings, not a wire: either stack plans and applies while the other is mid-apply, or absent, and neither destroy breaks the other's plan. The price of that independence is that the two carrying the API's URL must agree — a disagreement is not an apply error, it is a demo whose own fetches its own policy blocks. That describes this stack once it is applied, which it is not yet — until then the demo a visitor opens is not served from here at all, and the origin the API admits is not this distribution's; `.github/workflows/deploy-pages.yml`'s header owns #285's interim hosting arrangement and its teardown.
...
Five DynamoDB tables and four CloudWatch alarms. Four of the tables are [ADR 0002](../docs/adr/0002-storage-split.md)'s domain tables; the fifth, `abuse`, is [issue #29](https://github.com/TomBennett-Lloyd/cumulo/issues/29)'s per-IP limiter state per [ADR 0006](../docs/adr/0006-demo-abuse-protection.md), which lives in this stack because it is the same store, the same naming convention, and the same cost ceiling. The alarms are on `series` and `weather` only — the two tables the hourly pipeline writes, at its two ends: `weather` by the ingestion cycle, `series` by the forecast consumer draining the queue that cycle fills. Both are on-demand — `weather` since [#156](https://github.com/TomBennett-Lloyd/cumulo/issues/156), `series` since [#258](https://github.com/TomBennett-Lloyd/cumulo/issues/258). Every command runs from `infra/storage/`:
...
**B3. Confirm the billing mode the whole cost argument now rests on.** Read it back from AWS, not from Terraform's opinion of AWS:

```bash
ENV="$(terraform output -raw environment)"
for T in series weather; do
  aws dynamodb describe-table --table-name "cumulo-$T-$ENV" \
    --query 'Table.BillingModeSummary.BillingMode'
done
# expect: "PAY_PER_REQUEST" twice
```

The readback above is that confirmation: since [issue #258](https://github.com/TomBennett-Lloyd/cumulo/issues/258) moved `series` — the last table that did — **nothing in this stack draws on the Region's always-free capacity pool at all**. `infra/storage/tables.tf`'s header owns that posture, and the [storage stack's cost table](#storage-stack) is where the metered alternative is priced.

**Both tables are read back through `BillingModeSummary`, and that is the only field that answers the question.** An on-demand table still reports a `ProvisionedThroughput` block, with both figures zero, so a provisioned-capacity query would answer `W 0, R 0` for a healthy table and for a broken one alike — it cannot tell "on-demand" from "provisioned at nothing". A `PROVISIONED` answer for either table is that table's throttle class quietly restored: the burst shape that lost 6 of 12 locations in a live weather cycle (#156) and failed 12 of 24 forecast messages on `series` (#258), not a number that merely wants raising.

Then confirm the non-resource is genuinely absent. The check outlives the capacity it was written for — an empty list is now the assertion that nothing has re-provisioned a table and hung a scaler on it:
...
**B5. Smoke-test the adapters against the real tables.** The adapter unit tests prove what a mock can prove: which command inputs we build, and how we read the replies. This is the check that asks DynamoDB whether it agrees, and it now asks about all five tables — that the sparse by-location GSI's INCLUDE projection really carries every physics field the read parses back out of it (`sites`); that a `BETWEEN` whose upper bound is a bare `T#<timestamp>` really excludes the items at that timestamp, and that a batch write really lands every item and reports itself `complete` (`series`); that a transaction really lands the archive marker with its readings, and that the archive range is half-open at its upper bound too (`weather`); that `ADD` really accumulates a counter, that `if_not_exists` really refuses to move an expiry a second write asked to extend, and that a block whose instant has passed is reported as not blocking (`abuse`); and that a `begins_with` on a period prefix really excludes the neighbouring period (`metrics`). It also reads TTL back as _configuration_, with `DescribeTimeToLive`: ENABLED on `expiresAt` for `series`, `weather` and `abuse`, and DISABLED for `metrics`, whose rows are the published evidence the model comparison rests on and must never expire — a posture the run asserts against the deployed table rather than against `storage/tables.tf`, which is the only reason to ask AWS at all.

What it deliberately does not do is wait for a reap. TTL deletion is asynchronous and best-effort — days, not seconds — so a check that waited for an expired row to vanish would be a check that fails for the wrong reason; that same lag is why `AbuseAdapter.getBlock` compares `blockedUntil` to a clock instead of trusting a row's presence — an absent row is proof that nothing is blocked, but a present one is not proof that the block still holds — and the expired-block check proves that verdict against a row it has first shown to be still present.
...
Teardown is a project requirement, so it is stated rather than assumed. This stack has no ordering trap at all: its state lives in a bucket another stack owns, nothing here is another stack's foundation, and — unlike the Kinesis stream ADR 0004 replaced — every resource in it is free whether it is destroyed or forgotten. (A _left-enabled schedule_ is the caveat: it spends in the storage stack, not here — see "Whether to leave it up" above.)
...
**Whether to leave it up** is a real choice, unlike storage's. The stack costs $0 idle either way, but a stack that is up is *running*: it fetches from Open-Meteo every hour, spending quota (~288 calls/day against the free 10,000), metering the weather table's on-demand writes (≈ $0.30/month at the canonical fleet since #156) and, through the queue, the `series` writes the forecast consumer makes from them (≈ $1.78/month since #258) — both billed under storage, and the second the larger — and filling the queue with messages nothing is consuming until #136 lands. Leaving it running is the right default while #136 is being built and the wrong one afterwards if the queue is not being drained. Disabling the rule (`aws events disable-rule --name "cumulo-ingestion-hourly-$ENV"`) stops the cycles without destroying anything — but it is drift, and the next `terraform apply` re-enables it, which is the correct behaviour and worth knowing before it surprises you.

---

## Runbook: the api stack

One Lambda, one API Gateway HTTP API with its `$default` stage and integration, four routes — the catch-all plus the three declared write routes that carry a tighter 2 rps / burst 4 throttle ([ADR 0006](../docs/adr/0006-demo-abuse-protection.md), #29) — one log group, one execution role, two alarms, two Lambda permissions, one warmer schedule with two targets and its async invoke config ([#473](https://github.com/TomBennett-Lloyd/cumulo/issues/473)), and one deploy grant on the shared GitHub Actions role. That is [issue #14](https://github.com/TomBennett-Lloyd/cumulo/issues/14)'s infrastructure per [ADR 0005](../docs/adr/0005-fleet-api-hosting.md), plus #29's write throttles and #473's warmer. Every command runs from `infra/api/`:

```bash
cd infra/api
```

**This is the first stack in the platform that is reachable from the public internet**, and the one property to hold onto while reading the rest: the write endpoint is unauthenticated by design (ADR 0001 — auth is #30), so the stage throttle is what turns "we hope nobody hammers the demo" into an arithmetic bound of ≈ $36/month. It is two lines in `gateway.tf`, and an apply that dropped them would remove the bound — silently, for either of the two. `throttling_rate_limit` was a declared infra mirror (#133) until #296 retired the web fan-out whose launch rate it was held strictly above; with no client-side launch rate left to bound, that pair is gone from the mirror gate and dropping either line is now the half nothing catches. `gateway.tf`'s cost-guard comment owns that reasoning.
...
Expect **`Plan: 20 to add, 0 to change, 0 to destroy.`** — the function, its log group, the HTTP API, the integration, the `$default` route, the **three declared write routes** (`aws_apigatewayv2_route.write` is a `for_each` over three route keys, so it counts as three), the `$default` stage, the gateway's Lambda permission, the execution role, its inline policy, the two alarms, the deploy grant on `cumulo-github-actions`, and #473's five: the warmer rule, its two targets, the warmer's Lambda permission, and the function's async invoke config. Any other count means the configuration is not what this document describes; stop and find out why. The five data sources — `aws_caller_identity`, the existing `cumulo-github-actions` role, and three IAM policy documents (Lambda trust, execution, deploy) — are read rather than created and add nothing to the count.
...
**Read the warmer's payload too.** Both `aws_cloudwatch_event_target` blocks should show the same `input` — a payload-v2 event for `GET /v1/sites` with `sourceIp` `192.0.2.1` and **no `origin` header**. That absence is what stops the synthetic event ever being admitted to a write route, and `infra/api/warmer.tf` argues it; a plan showing an `origin` here is a plan that opens ADR 0006's origin check from the inside.
...
```bash
terraform state list   # expect 25 lines — the 20 resources plus the 5 data sources
```
...
**B7. Prove the warmer.** The rule is enabled by the apply, so the only question is whether its ticks are reaching the function — and whether they are reaching **two** environments rather than one. Confirm the wiring first:

```bash
aws events describe-rule --name "cumulo-api-warmer-$(terraform output -raw environment)" \
  --query '{Schedule:ScheduleExpression,State:State}'
# expect: cron(0/5 * * * ? *), ENABLED
aws events list-targets-by-rule --rule "cumulo-api-warmer-$(terraform output -raw environment)" \
  --query 'Targets[].Id'
# expect: two ids — warm-first and warm-second — both on the api function
```

**Then replay one target's own payload and read the status back.** A warmer aimed at a path the API no longer routes still warms the runtime — a 404 is an invocation like any other — but stops opening the DynamoDB connection, which is half of what it is here for, and nothing anywhere goes red. `warmer.tf` names that as an unmechanised code↔Terraform mirror; this is the readback that catches it, and it is worth re-running after any change to the route table in `apps/api/src/main.ts`:

```bash
RULE="cumulo-api-warmer-$(terraform output -raw environment)"
aws events list-targets-by-rule --rule "$RULE" --query 'Targets[0].Input' --output text > /tmp/cumulo-warmer-event.json
aws lambda invoke --function-name "$(terraform output -raw function_name)" \
  --payload "fileb:///tmp/cumulo-warmer-event.json" /tmp/cumulo-warmer-response.json \
  --query 'StatusCode'
python3 -c 'import json;print(json.load(open("/tmp/cumulo-warmer-response.json"))["statusCode"])'
# expect: 200 from the invoke (Lambda accepted it) and 200 from the body
# (the handler routed it). A 404 in the body is the drift this step exists for.
```

This is a synchronous invoke, so it reports what the asynchronous ticks cannot: EventBridge discards the response, and a rule delivering 404s looks identical to one delivering 200s on every metric below.

Then ask the log group how many execution environments answered. Each environment writes its own log stream, so counting distinct streams counts containers — but **the window has to start after the first tick**, because the first tick's two invocations are cold by definition and would put two unavoidable cold starts inside it. Wait about twenty minutes from the apply, so that the 15-minute lookback below lies entirely in the warm period:

```bash
FUNCTION_NAME="$(terraform output -raw function_name)"
QUERY_ID="$(aws logs start-query --log-group-name "/aws/lambda/$FUNCTION_NAME" \
  --start-time $(($(date +%s) - 900)) --end-time $(date +%s) \
  --query-string 'filter @type = "REPORT" | stats count() as invocations, count_distinct(@logStream) as containers, count(@initDuration) as coldStarts' \
  --query queryId --output text)"
sleep 10 && aws logs get-query-results --query-id "$QUERY_ID" --query 'results'
# expect: invocations 6 or 8 (a 15-minute window holds 3 or 4 ticks depending on
# where it falls against :00/:05/:10, × 2 targets), containers 2, coldStarts 0
```

`containers 1` means the two deliveries were serialised rather than dispatched together — the residual `warmer.tf` names. One tick landing that way is expected occasionally; every tick landing that way is a finding. A `coldStarts` above zero in a window that starts after the first tick means an environment was recycled mid-window, which Lambda does on its own schedule; it is a finding only if it repeats.

**The same query is how the fix is judged over time.** Re-run it with a `--start-time` a month back and `by ispresent(@initDuration)` instead: before #473 every visit produced a cold row, and afterwards `coldStarts` should be the deploys and the occasional environment recycle, not the visitors. **That comparison has a shelf life**, and the log group sets it: `retention_in_days = 30` in `infra/api/lambda.tf`, so a 30-day lookback sits exactly on the retention edge and one run more than a month after the apply returns an empty "before" half with nothing to say it was dropped rather than empty. Run it within a month of the apply, or read the "before" figures off this ticket instead.

**What this query cannot do is separate the warmer's invocations from real ones**, and nothing in the log group can: the API writes no application line on a request that goes well, so a warm `GET /v1/sites` from a visitor and one from the rule are the same three platform lines. The payload's markers (`192.0.2.1`, `user-agent: cumulo-warmer`) identify it to anyone reading the rule or doing a test invoke, and the split at runtime is read from metrics instead — `AWS/Events` `Invocations` on the rule is the warmer exactly, `AWS/ApiGateway` `Count` on the api is real traffic exactly, and `AWS/Lambda` `Invocations` is the sum:

```bash
aws cloudwatch get-metric-statistics --namespace AWS/Events --metric-name Invocations \
  --dimensions Name=RuleName,Value="cumulo-api-warmer-$(terraform output -raw environment)" \
  --start-time "$(($(date +%s) - 3600))" --end-time "$(date +%s)" \
  --period 3600 --statistics Sum --query 'sum(Datapoints[].Sum)'
# expect: 24 — 12 ticks × 2 targets — once the rule has been up a full hour.
# Summed across datapoints on purpose: an unaligned hour splits into two partial
...
# expect: []
aws events list-rules --name-prefix cumulo-api-warmer --query 'Rules[].Name'
# expect: []
...
**What does not survive the round trip is the endpoint.** The api id is server-assigned, so a re-apply hands out a new one and every copy taken from `api_endpoint` is now stale — the `VITE_API_BASE_URL` repo variable and `api_origin` in `infra/web`'s tfvars both. Neither is a Terraform reference, so nothing fails at plan time; the web demo simply calls an address that is gone, or has its own CSP refuse the new one. Re-run the web runbook's [A1](#phase-a--configure-and-plan-the-distribution) and [B2](#phase-b--apply-publish-and-prove-the-edge) after any spin-up that reassigns the id. `infra/api/outputs.tf` carries the same list beside the output itself.

**Whether to leave it up.** Since #473 this stack does do something while nobody is looking: the warmer invokes the function twice every five minutes, which is the whole point of it — the visitor whose arrival nobody can predict is exactly the one it exists for. An idle API is therefore still free, but no longer inert, and the cost table's warmer note owns what that spends (≈ $0.005/month, on the storage stack's meter). The reason to think about leaving it up is the older one, unchanged: it is public. Leaving it up means leaving an unauthenticated write endpoint on the internet. What bounds it is the layered posture [ADR 0006](../docs/adr/0006-demo-abuse-protection.md) records — an `Origin` check and a per-IP limiter with an auto-block inside the function, the 2 rps write throttle and the 10 rps stage throttle at the gateway, and a hard cap of 40 user sites with oldest-first eviction in the data model. That is an accepted risk for a portfolio demo and a deliberate one; it is not an oversight.
...
**B4. Prove the 6× floor still holds.** Not drift between the two Terraform files — `check:infra-mirrors` holds those two declarations to the floor on every `verify` run. What this step proves is that the invariant spanning two stacks survived the trip into AWS: two stacks apply independently, so a floor that holds in source can still be violated in the account by one of them not having been applied:
...
One private S3 bucket, its public-access block and its origin-access-control policy, one CloudFront origin access control, one distribution, one response headers policy attached to that distribution's single behaviour, and one deploy grant on the shared GitHub Actions role — seven resources and no alarms: [issue #144](https://github.com/TomBennett-Lloyd/cumulo/issues/144)'s hosting slice in full, plus the security headers [issue #176](https://github.com/TomBennett-Lloyd/cumulo/issues/176) hung on it. Every command runs from `infra/web/`:
...
3. **The api stack applied**, if the demo is to show real data. Not a Terraform prerequisite in either direction — this stack plans and applies with no API in the account — but B2 publishes `VITE_API_BASE_URL` from `infra/api`'s output, and without it `deploy-web.yml` fails its preflight rather than shipping anything — the in-memory demo fleet a build without it would carry is exactly what that guard exists to keep off a hosted page. That same `api_endpoint` output also populates `api_origin` here, in [A1](#phase-a--configure-and-plan-the-distribution), so the API is now wanted before the _plan_ and not only before the build — and the two copies must agree, because the policy admits exactly the one origin `api_origin` names and nothing else.
...
`api_origin` defaults to empty, and that default exists for exactly one situation: **planning this stack before `infra/api` is applied**. It is not a deployment mode. Neither deploy path will publish a build without an API base URL — `deploy-web.yml`'s preflight step fails the run on an empty `VITE_API_BASE_URL`, and `deploy-pages.yml` carries a job-level `if: vars.VITE_API_BASE_URL != ''` and skips entirely — so any distribution the deploy workflow ever reaches needs this set, and a distribution serving content with it still empty is the quiet failure the next paragraph describes rather than a supported configuration. Leave it out to walk A4's plan through on its own; set it before anything is served. Derive the value — never retype it — and put it in `web.auto.tfvars`:

```bash
terraform -chdir=../api output -raw api_endpoint
```

That is the same command, against the same output, that [B2](#phase-b--apply-publish-and-prove-the-edge) pipes into the `VITE_API_BASE_URL` repo variable, and the two must carry the same string. They are one value travelling two ways: B2's copy is baked into the build so the SPA knows where to call, this one is rendered into the `connect-src` of the policy `security-headers.tf` builds so the browser permits the call. Deriving both from the command above is what makes them agree by construction; the ways they still come apart are retyping one of them, setting this one and forgetting B2, or re-applying the api stack between here and there, which reassigns the api id and invalidates whichever copy was captured first.

**A disagreement is not an apply error, and that is the whole reason it is worth this paragraph.** The plan is clean, the apply succeeds, the deploy run goes green, the page loads and the map draws — and every API call the demo makes is refused by the demo's own policy, reported in the visitor's browser console and nowhere an operator is looking. Leaving this empty against a build that _does_ have an API base URL fails exactly the same way. `variables.tf` argues the other half of the rule at length: the exact host, never a wildcard, because `https://*.execute-api.<region>.amazonaws.com` would admit every API Gateway in the region as a destination this page may send data to.
...
Expect **`Plan: 7 to add, 0 to change, 0 to destroy.`** — the bucket, its public access block, its bucket policy, the origin access control, the response headers policy, the distribution, and the deploy grant on `cumulo-github-actions`. Any other count means the configuration is not what this document describes; stop and find out why. The five data sources — `aws_caller_identity`, the `Managed-CachingOptimized` cache policy, the existing `cumulo-github-actions` role, and the two IAM policy documents (the bucket's origin grant and the deploy grant) — are read rather than created and add nothing to the count.
...
**Then read the security headers back off the edge.** The response headers policy is attached to the distribution's one behaviour, so these ride on every response it serves — which is what makes `/` a sufficient place to ask, and a missing header a policy that is not attached rather than a path that is special:

```bash
curl -sI "$CF_URL/" | command grep -i '^strict-transport-security'
# expect: max-age=31536000 — one year, and deliberately no includeSubdomains and no preload
# (security-headers.tf argues both, and its restatement ledger names this line as what asserts the number)

curl -sI "$CF_URL/" | command grep -i '^x-content-type-options'
# expect: nosniff

curl -sI "$CF_URL/" | command grep -i '^content-security-policy'
# expect: content-security-policy.tftpl's directives on one line, joined with "; "
```
...
Keep `backend.hcl` and `web.auto.tfvars` — both are still correct for the next spin-up, with one entry to re-check rather than assume. `api_origin` names the api stack's server-assigned host, so if _that_ stack was destroyed and re-applied while this one was down, the kept value points at an api id that no longer exists and the spun-up demo's own calls are blocked by its own policy. Re-derive it from `terraform -chdir=../api output -raw api_endpoint` in that case, exactly as A1 does. To spin back up, run [Phase A](#phase-a--configure-and-plan-the-distribution) then [Phase B](#phase-b--apply-publish-and-prove-the-edge) back to back.

**Whether to leave it up.** Standing cost is ~$0.00004/month — the stored build, and nothing else — and it is the demo's public face, so the default is yes. Three stacks are busy while nobody is looking — ingestion's schedule, the api stack's warmer since #473, and the forecast stack's long-poll of ingestion's queue — and this is not one of them: no schedule, no queue, no Open-Meteo quota. Unlike the api stack it exposes no write endpoint of its own either. The one honest reason to think about it is the one the cost table states plainly: this is the only public surface in the platform with no throttle, so a hot-linked or hammered distribution is bounded by the free terabyte and the budget alarm rather than by arithmetic.
...
Three conventions hold across every table below (the third has one recorded gap: the api stack does not cost its own `series` reads, [#243](https://github.com/TomBennett-Lloyd/cumulo/issues/243) — it has carried a DynamoDB row since #473, but that row is the warmer's reads on `sites` and not the visitor traffic #243 is about). They are stated once here rather than restated per stack, because the previous edition of this document quietly used two different ones and the tables disagreed with each other:
...
**Two rows are sized differently, and say so.** Since [issue #156](https://github.com/TomBennett-Lloyd/cumulo/issues/156) `weather` is on-demand and since [issue #258](https://github.com/TomBennett-Lloyd/cumulo/issues/258) `series` is too, so both tables' writes are metered rather than drawn from an allowance — and the honest number for a metered line is the one the running system produces, not the planning envelope. They are therefore priced at the **canonical fleet** (12 distinct locations, the sizing the ingestion and forecast stacks' tables below use); ADR 0002's envelope and the caps ADR 0006 sets are priced alongside them in the notes. Every other row is still the ADR's sizing.

| Resource group                                                                     | Billing basis                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Estimate                                       |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| **On-demand capacity — `series`** (the forecast fan-out's writes)                  | **Metered, no free allowance** since #258: $0.705/M write request units. Each cycle turns every location's horizon into forecast items — ~2,880/cycle at the canonical fleet — and, since #494, one fleet roll-up partial per location-hour on top: `12 × 48 = 576`, so `3,456 × 730` ≈ **2.52 M write units/month**. Reads bill at $0.1415/M: a dashboard load spends ~43 units on this table — ~18 for one Query of the `#FLEET` partition, plus ~25 covering every site's partition for the simulated actuals whose fan-out ADR 0009 leaves in place — plus ~2 over the `FLEET` partition on `sites`, so ≈ 45 a load, thousands of loads to a cent. The per-load arithmetic is owned by the `series` section of `storage/tables.tf` (#264 doubled it, #494 replaced the forecast half with one Query; ADR 0002's ≈ 27 is amended). **$0 whenever the schedule is not running** — driven by the forecast stack's consumer (its DynamoDB row points here) and the api's reads (the api stack's own row for its visitor-driven `series` reads is pending — #243; its DynamoDB row today costs #473's warmer reads on `sites` and nothing on this table) | **≈ $1.78/mo** (≈ $6.28 at #29's 100-site cap) |
| **On-demand capacity — `weather`** (the hourly cycle's writes)                     | **Metered, no free allowance**: $0.705/M write request units. Each cycle writes every location's 48-hour horizon, so `12 × 48 × 730` ≈ **0.42 M write units/month**. Reads bill at $0.1415/M and are noise beside that. **$0 whenever the schedule is not running** — driven by the ingestion stack's schedule (its DynamoDB row points here)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **≈ $0.30/mo** (≈ $1.28 at 52 locations)       |
| **On-demand capacity — everything else** (`sites` + both GSIs, `metrics`, `abuse`) | Same rates, request-shaped load — tens of thousands of requests/month, of which the api stack's warmer is the only part that arrives without a visitor: 17,280 Queries on `sites`, ~34,560 read units, ≈ $0.005 (#473, and the api cost table's DynamoDB row has the arithmetic)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **$0.00/mo**                                   |
| **Storage** (all five tables)                                                      | ~3.5 GB inside the always-free **25 GB**; past that allowance it is $0.283/GB-month. Whether that allowance covers on-demand tables is the one thing ADR 0002 could not confirm, and since #258 every table here is on-demand — see the note below for what it would cost if not                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **$0.00/mo**                                   |
...
| **Standing total**                                                                 | Every charged row here is an activity row, so an idle stack bills only its stored bytes — and those are inside a permanent allowance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **$0.00/mo**                                   |
| **Total, schedule running**                                                        | The standing total plus both capacity activity rows, at the canonical fleet — `series` ≈ $1.78 and `weather` ≈ $0.30                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **≈ $2.08/mo**                                 |

**The at-rest rows are still genuinely zero. The total is not — and this stack is the exact inverse of the two others in this document that carry a non-zero figure.** Bootstrap's ≈ $0.0005 and web's ≈ $0.00004 are _standing_ totals: bytes that bill while nothing happens, with their activity rows excluded. Here the standing figure is $0 and the charge is entirely activity. The alarms and the stored bytes sit inside allowances and bill $0 rather than rounding down to it — and this is emphatically not a stack with nothing that bills for existing, because DynamoDB charges for stored bytes whether or not anything ever reads them, and **~3.5 GB** is exactly such a line. What makes that one $0.00 is that the always-free **25 GB** absorbs it with ~86% of the pool still spare, and that allowance does not expire.

What moved is the activity, in two steps. Request units have no perpetual free allowance: since #156 the hourly cycle's weather writes are billed at that rate, and since #258 the forecast fan-out's `series` writes are too. **≈ $1.78/month** between them is a real charge, not a sub-cent amount displayed as zero, so it is carried into the total rather than rounded away — and the larger half of it is the table that used to be the whole reason this stack claimed a free allowance at all. The remaining on-demand tables keep the older shape — a sub-cent amount at demo volume, the same as the bootstrap stack's request line, which is an activity row too and is the row that stack's standing total excludes; it makes no difference here either way.

**The property that survives, and it is the one that was ever load-bearing: the standing bill is still $0.** Every charged row in this stack is an activity row. An idle stack — no cycle running, nobody looking — costs nothing at all, indefinitely, with the tables and all their data still sitting there, because nothing in it is priced by the hour and the one line priced for merely existing is inside a permanent allowance. Since #473 "nobody looking" has one caller in it, and it is a rounding error rather than an exception: the api stack's warmer reads `sites` every five minutes for ≈ $0.005/month, and stops when that rule is disabled like every other activity row here. **≈ $1.78/month is what this stack costs for doing its job**, and it is 1.8% of the ~$100 ceiling. The headline was never "$0 no matter what happens"; it was "$0 for standing still, and a metered fraction of a dollar for running" — the fraction is now a small multiple of a dollar, and it is the price of two throttle cliffs that no free allocation could have absorbed.

Notes on what would change that:

- **The `series` row at all three fleet sizes, since the row itself only carries two.** The arithmetic is `series items per cycle × 730 × $0.705/M`, and a site contributes `48 × 2 = 96` of them plus one generation actual once #264's producer runs, while #494's roll-up adds a term that scales with **locations** rather than with sites — `locations × 48` partials a cycle, so the three sizes need their location counts and not only their site counts: **~3,456/cycle ≈ 2.52 M units ≈ $1.78/mo** (the canonical fleet — ~30 sites over 12 locations, `12 × 48 = 576` partials, and the only one of the three that is currently real), **5,426 ≈ 3.96 M ≈ $2.79/mo** (ADR 0002's ~50-site envelope, actuals included, which the rest of this table is sized against; that envelope states no location count, so the canonical 12 is assumed and the partial term is the same 576 — a fleet spread wider costs more here), **12,196 ≈ 8.90 M ≈ $6.28/mo** (ADR 0006's 100-site cap, whose worst case `packages/shared/src/site.ts` states as **≤ 52 locations, ≤ 100 sites**, so `52 × 48 = 2,496` partials — the number to hold against the ceiling). It is the platform's largest single line by an order of magnitude and still ~6% of the ceiling at a fleet four times the size the demo has ever run — which is the whole reason the flip needed no `on_demand_throughput` ceiling, for the same argument the weather note below makes.
- **The weather row at all three fleet sizes, since the row itself only carries two.** The arithmetic is `locations × 48 horizon hours × 730 cycles × $0.705/M`, and it scales linearly because the horizon and the cadence are fixed: **12 locations ≈ 0.42 M units ≈ $0.30/mo** (the canonical fleet, and the only one of the three that is currently real), **~30 ≈ 1.05 M ≈ $0.74/mo** (ADR 0002's planning envelope, which the rest of this table is sized against), **52 ≈ 1.82 M ≈ $1.28/mo** (ADR 0006's worst case, the number to hold against the ceiling). The spread is the point: the worst case this stack can be driven to is still under a dollar and a half, which is why the flip needed no ceiling — `tables.tf` sets no `on_demand_throughput` maximum, because a request-unit cap is just the throttle cliff of #156 wearing a different name, and the cost backstop is the bootstrap stack's `aws_budgets_budget.monthly_cost_ceiling` instead — the same backstop the web stack leans on.
- **The free capacity allowance is a hard edge, not a discount — and this stack no longer stands anywhere near it.** Crossing 25 WCU or 25 RCU in the Region bills the excess at $0.000735/WCU-hour and $0.000147/RCU-hour. Nothing here is provisioned, so nothing is close; `tables.tf` still has no auto-scaling and still says so at length, because an `aws_appautoscaling_target` is the one change that crosses that edge without appearing in anyone's plan review, and the ticket that would reach for one is the next batch-shaped table rather than anything already here.
- **The pool is Region-wide and shared, and since #258 it is entirely unclaimed** — 25 WCU / 25 RCU, none of it drawn. That is not a growth reserve either; the standing rule (a new table defaults to on-demand unless its load is batch-shaped) is what stops it being needed. `abuse` is that rule working from the start: on-demand, so it never drew on the pool and never changed a number in this table. `weather` and then `series` are the same rule working in reverse, and learning it twice cost a live incident each time: both _were_ batch-shaped by the rule's test, and provisioned anyway failed it, because the rule sizes sustained rate and the thing that broke was burst shape. Two for two is the fact a third batch-shaped table has to argue against.
...
- **The honest inversion, now settled by events rather than by argument.** At list price the 14/21 allocation `series` has just given up would have cost `14 × $0.000735 × 730 ≈ $7.51` in write capacity plus `21 × $0.000147 × 730 ≈ $2.25` in read — **≈ $9.76/month** — against on-demand for the same work: `series` takes 4,850 of the cycle's 6,290 writes (the 4,800 forecast items plus 50 actuals), so `4,850 × 730 ≈ 3.54 M` write units at $0.705/M ≈ **$2.50**. Provisioned was **3.91×** the on-demand bill and was taken anyway, because the first 25 units were free — and that is the side the platform now pays in cash, the other having only ever been notional. ADR 0002's own pair — **≈ $11.30** against **≈ $2.88**, a **3.92×** — covered the whole pre-#156 pool, both tables' capacity together, and it computed them at us-east-1 rates on the same 730-hour month. The ADR is restated-with-a-correction here rather than amended, on the precedent the api stack's log-ingestion note sets below: its conclusion is the **ratio**, and the ratio survives both corrections at once. Changing Region moved every rate in the comparison by about the same 13% (12.8% to 13.2%; PITR, outside the ratio, moved +10%), and dropping `weather` out of both sides removed roughly a quarter of each — a ratio notices neither. What the corrections move is the absolute pair, and those are the numbers a reader would quote.
- **Neither table left the free tier on price.** A 7% duty cycle is exactly what on-demand pricing exists to serve; provisioned won on `series` and `weather` alike only because the tier is free, and ADR 0002 says so rather than pretending it is the better engineering answer. Neither flip happened because that arithmetic changed — it still favours provisioned on paper, and the two together gave up a free line for a ≈ $1.78 one. They happened because free capacity that cannot absorb a 25-item `BatchWriteItem` burst is not capacity the workload can use: the alarm fired live on `weather`, where 6 of 12 locations failed to publish ([#156](https://github.com/TomBennett-Lloyd/cumulo/issues/156)), and then on `series`, where 12 of 24 forecast messages failed on first delivery and only SQS redelivery kept the data ([#258](https://github.com/TomBennett-Lloyd/cumulo/issues/258)) — and no pool-fundable WCU number fixed the burst shape either time. That is the whole of the trade: **≈ $1.78/month to convert two cliffs into rates**, on the escape hatch `tables.tf` and ADR 0002's reversibility section both named in advance.
- **If the always-free 25 GB turns out not to cover on-demand tables, the storage row above is an exposure of ≈ $0.99/month.** ADR 0002 records this as the one thing it could not confirm from AWS's docs, and it noted the question was moot because the two tables holding essentially all the data were both provisioned. #156 made half of that untrue and the retained-bytes argument covered the gap: `weather`'s forecast rows are keyed `(locationId, FORECAST#T#<validTime>)` and every cycle rewrites the same 48-hour window shifted one hour, so what is _retained_ is one row per location-hour rather than one per write — megabytes, cents either way. #258 removes the argument rather than extending it, because the ~3.5 GB is dominated by `series` and `series` is now on-demand too. So the honest form is a conditional, not a dismissal: if the allowance is provisioned-only, ~3.5 GB at $0.283/GB-month is **≈ $0.99/month** on top of the activity rows. It is answerable from one month of Cost Explorer once the tables have carried real data for one, and that is cheaper than discovering it on a bill.
...
- **Nothing here has an hourly rate**, which is the property to preserve, and neither flip spent it — an on-demand table has no rate at all until something writes to it. That is the difference between this stack's ≈ $1.78 and a clock-driven charge: it is bounded by what the fleet does, and it goes to zero when the fleet does nothing. The largest line is now `series` at 1.5% of the ~$100 ceiling, with the whole stack at 1.8% while running; a VPC, a NAT gateway, or a database instance would be the whole budget — the comparison ADR 0002 turned on, and the one this stack still wins by two orders of magnitude.
...
| **DynamoDB writes** (`cumulo-weather` via the cycle)               | 12 locations × 48 hourly rows ≈ 576 write units/cycle → **~0.42 M/month**, metered on-demand since #156. This stack adds load _and_ a line — the line lives in the **storage** cost table's weather row, because storage owns the resource        | **$0.00/mo here; drives ≈ $0.30/mo under storage** |
...
- **A forgotten stack is nearly free, and it is not inert.** It is one of the two stacks with a **clock of its own** — the api stack's warmer joined it at #473 — and this is by far the more expensive of that pair, its ≈ $2.08/month against that one's ≈ $0.005. The forecast stack is busy while nobody is looking as well, and is deliberately not counted here: its event source mapping polls _this_ stack's queue, so what it does unprompted is wait rather than work. The polling itself does not stop when this schedule stops — an ESM holds its floor of long-polling connections against an empty queue, which is why the ~657,000 receives a month on the forecast stack's event-source-mapping row are a standing term — but nothing downstream of it fires, so its Lambda invocations and its `series` writes go to zero and this stack's do not. An enabled schedule spends ~288 Open-Meteo calls/day against the 10,000/day free tier, grows an unconsumed queue, and — since #156 flipped `cumulo-weather` to on-demand — meters real write units into the storage stack's weather row, ≈ $0.30/month at the canonical fleet. That is one of the platform's two recurring charges and this schedule drives it directly; it drives the other one indirectly, because every message it enqueues becomes `series` writes — forecasts, and since #494 the fleet roll-up partials beside them — on the storage stack's `series` row, ≈ $1.78/month since #258 (both lines live in the storage cost table, and the forecast stack's own row points at the second). Within this stack the only spend is the log group, and `retention_in_days = 30` is what makes that a rolling ~12 MB rather than an unbounded pile — see the teardown section above.
...
The figures are [ADR 0005](../docs/adr/0005-fleet-api-hosting.md)'s, restated here because a cost table that lives only inside a decision record is not somewhere an operator looks. **This is the one stack whose volume is partly a schedule and partly not.** The traffic half is costed twice, as it always was: at the expected regime (a portfolio demo — order 10,000 requests/month including Swagger UI's assets) and at the bound (the stage throttle pegged continuously for a 30-day month, 25.92M requests). Underneath both sits a fixed **17,280 invocations/month** from [#473](https://github.com/TomBennett-Lloyd/cumulo/issues/473)'s warmer — `infra/api/warmer.tf`, two direct invokes every five minutes, which reach the function without reaching the gateway and so appear on the Lambda rows below and on none of the gateway ones.
...
| **Lambda invocations** (`aws_lambda_function.api`)                         | ~10,000/month of traffic plus the warmer's 17,280 — **~27,000/month**, 2.7% of the always-free **1,000,000 requests/month**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **$0.00/mo**                                        |
| **Lambda compute**                                                         | 256 MB × ~100 ms is 0.025 GB-s/request, so the always-free **400,000 GB-seconds** covers 16M requests/month. The warmer's share is `17,280 × 0.25 GB × 54 ms` ≈ **233 GB-s**, 0.06% of the allowance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **$0.00/mo**                                        |
| **Warmer schedule** (`aws_cloudwatch_event_rule` + 2 targets + permission) | EventBridge scheduled rules and their invocations are **not charged** — $0 by pricing, not by allowance, the figure the ingestion stack's Schedule row above owns. Its invocations ride the two Lambda rows above and the CloudWatch row below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **$0.00/mo**                                        |
| **DynamoDB reads** (`cumulo-sites` via the warmer)                         | One `GET /v1/sites` Query per ping ≈ 2 read units, so `17,280 × 2` ≈ **34,560 units/month** at $0.1415/M ≈ $0.005. This stack adds load _and_ a line — the line lives in the **storage** cost table's "everything else" row, because storage owns the resource                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **$0.00/mo here; drives ≈ $0.005/mo under storage** |
| **CloudWatch logs — stored bytes** (30-day retention on one group)         | **Bills for existing**: ~$0.03/GB-month retained. **No application line rides on a request that goes well**; any request logs at most a line or two on its failure paths (contention, deadline — the read routes' deadline sinks included). So the billed unit is **3 lines/invocation** — Lambda's `START`/`END`/`REPORT`, precisely what ADR 0005's ~250 B/invocation measures — and the failure-path lines are a rounding error against traffic dominated by reads and Swagger UI assets. The billed unit is the invocation, so the warmer's pings are counted here too: **~27,000 invocations × ~250 B ≈ 6.8 MB/month**, ~$0.0002/mo at list — inside the account's free **5 GB** of storage and the free **5 GB** of ingestion | **$0.00/mo**                                        |
...
**What #473 changed is not the total but the meaning of "idle".** A forgotten api stack is no longer inert: the warmer's rule fires twice every five minutes whether or not anybody is looking, which is the property the ingestion stack used to hold alone — a clock of its own, firing unprompted. The forecast stack's long-poll of ingestion's queue is activity with nobody looking too, but what it does unprompted is wait: its polling floor stands whether or not ingestion is running, and everything downstream of it — the invocations, the `series` writes — fires only when ingestion's schedule hands it something. The warmer answers to nothing, which is the property being named. It buys nothing back in this table — every row it touches is still $0 — and it costs ≈ $0.005/month on the storage stack's meter, half a cent to keep the demo's first paint warm. The lever if that is ever unwanted is the rule's own `state`: disabling it takes the last two rows to nothing without moving a resource.
...
- **The throttle is the bound.** Delete `default_route_settings` from `infra/api/gateway.tf` and the worst case stops being finite — silently. `throttling_rate_limit` was a declared infra mirror (#133), held strictly above the web fan-out's launch rate, so removing it exited 2 at `pnpm verify`; #296 retired the fan-out and with it the launch rate the bound was against, so that pair is gone and neither line reds anything now. Deleting either goes unnoticed, and nothing else in the platform would notice either until the bootstrap stack's budget alarm fired at 50% of $100. Those two numbers belong in review whenever this stack changes.
...
- **The warmer's arithmetic is the cadence and the target count, and nothing else.** `12 ticks/hour × 2 targets × 24 × 30 = 17,280` invocations a month; halving the cadence or dropping to one target halves it, and neither would move any figure in this table off $0 — the allowances are two and three orders of magnitude away. What would move something is the DynamoDB row: it scales with the same number, so a one-minute cadence is ≈ $0.025/month rather than ≈ $0.005. `maximum_retry_attempts = 0` in `infra/api/warmer.tf` is what holds the count to one invocation per target per tick rather than up to three. It is exact on Lambda's side of the hand-off and not on EventBridge's: `aws_cloudwatch_event_target` carries its own `retry_policy`, left at the default, which re-attempts a delivery EventBridge itself could not make. That bites only when the hand-off fails, so it moves no figure here off $0 — but the count is "one per target per tick in the ordinary case", not "one, always". The warmer gets **no alarm of its own**, deliberately: all ten of the platform's allowance are spent (see [CloudWatch alarm budget](#cloudwatch-alarm-budget)), and a schedule that stops firing costs nothing and degrades to the behaviour this stack had before #473.
- **Nothing here has an hourly rate**, the property all seven stacks preserve. The warmer is a clock, not an hourly rate: it bills per invocation, so a disabled rule bills nothing at all.
...
| **DynamoDB writes** (`cumulo-series` via BatchWriteItem)           | ~3,456 items/hour — ~2,880 forecast items plus #494's 576 fleet roll-up partials → **~2.52 M write units/month**, metered on-demand since #258. This stack adds load _and_ a line — the line lives in the **storage** cost table's `series` row, because storage owns the resource | **$0.00/mo here; drives ≈ $1.78/mo under storage** |
| **CloudWatch logs — stored bytes** (30-day retention on one group) | **Bills for existing**: ~$0.03/GB-month retained. **11 billed lines/invocation × ~8,760 invocations × 1 KB ≈ 96 MB/month** (census below), so ~$0.0029/mo at list — inside the account's free **5 GB** of storage and the free **5 GB** of ingestion                               | **$0.00/mo**                                       |
...
- **The log-group size above is a bound, not a measurement — and the census is written out so it can be checked against the source.** An invocation emits `forecast.message.outcome` and `forecast.batch.summary` (`apps/forecast/src/handler.ts`), one of each because `batch_size = 1` in `infra/forecast/event-source.tf` makes a batch a single record; one `forecast.actuals.outcome` (`apps/forecast/src/simulate-actuals.ts`) per site of the location that record speaks for, which is five in the canonical fleet; one `forecast.fleet-rollup.outcome` (`apps/forecast/src/fleet-rollup-write.ts`, #494) for the location itself; and Lambda adds its own `START`, `END` and `REPORT`. That is **11 lines per invocation**, ~96,400 a month, priced the way ingestion's are — 1 KB per line, rounded up from the ~250 bytes per _invocation_ that ADR 0005's ~6.5 GB/month at 25.92M requests works out at. So **11 × 8,760 × 1 KB ≈ 96 MB/month**: the platform's largest log line by volume, and still under 2% of the account's free 5 GB. Note which way `batch_size` moves it — at a batch of _n_ the count is _n_ × (2 + sites at its location) + 4 lines per invocation rather than 11_n_, so raising it lowers the bytes per message. And note which term the fleet moves: the per-site half is `sites × 730` a month whatever the fleet's shape, so the sensitivity is to **locations**, not to sites. Spreading the same fleet over more locations buys more invocations and therefore more of the six fixed lines each — co-location is a log-volume decision as well as an Open-Meteo-quota one.
- **`maximum_concurrency` is still not a cost guard, and since #258 it is barely a throttle guard either.** Raising it does not cost more — the same messages are processed either way, and on-demand meters the units rather than the rate, so concurrency changes when they are spent and not how many. What it used to bound was the burst arriving at `series`' provisioned ceiling; that ceiling is gone, and what remains above this fan-out is DynamoDB's own per-table and per-partition request limits, orders of magnitude away. The value is a consumer-side decision now (`infra/forecast/event-source.tf` owns it) rather than a capacity one.
...
| **Distribution, OAC and response headers policy existence** (`aws_cloudfront_origin_access_control`, `aws_cloudfront_response_headers_policy`) | No hourly rate, no per-distribution fee, no charge for an origin access control or a response headers policy — both are configuration attached to a distribution rather than resources with a price. The header mechanisms that _would_ bill, CloudFront Functions and Lambda@Edge, are exactly what setting headers this way avoids | $0.00/mo                |
...
| Stack       | Alarms | Which                                                                                                                          |
| ----------- | -----: | ------------------------------------------------------------------------------------------------------------------------------ |
| `storage`   |      4 | `cumulo-{series,weather}-<env>-{read,write}-throttle` — two resources expanded by `for_each` over the two cycle-written tables |
