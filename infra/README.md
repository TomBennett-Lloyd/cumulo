...
Then confirm the non-resource is genuinely absent. This check still matters, and it now guards `series` alone — an empty list here is the assertion that nothing can scale it past the free tier:
...
**One row is sized differently, and says so.** Since [issue #156](https://github.com/TomBennett-Lloyd/cumulo/issues/156) `weather` is on-demand, so its writes are metered rather than drawn from an allowance — and the honest number for a metered line is the one the running system produces, not the planning envelope. It is therefore priced at the **canonical fleet** (12 distinct locations, the sizing the ingestion stack's table below uses); ADR 0002's ~30-location envelope and ADR 0006's 52-location worst case are priced alongside it in the notes. Every other row is still the ADR's sizing.

| Resource group                                                                     | Billing basis                                                                                                                                                                                                                                                                                                                                 | Estimate                                 |
| ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **Provisioned capacity** (`series` 14 WCU / 21 RCU)                                | 14 WCU / 21 RCU against the always-free **25 WCU / 25 RCU per Region**, which does not expire after twelve months — figures owned by `infra/storage/tables.tf`; the load is driven by the forecast stack's writes and the api's reads (the api's own return row is pending — #243)                                                            | **$0.00/mo**                             |
...
- **The pool is Region-wide and shared.** With `series` alone in it, 14/21 of 25/25 leaves **11 WCU / 4 RCU**. That slack is not a growth reserve — the standing rule (a new table defaults to on-demand unless its load is batch-shaped) is what stops it being needed. `abuse` is that rule working from the start: it is on-demand, so it neither draws on the 25/25 nor changes a number in this table. `weather` is the same rule working in reverse and it cost something to learn — it _was_ batch-shaped by the rule's test, and provisioned anyway failed it, because the rule sizes sustained rate and the thing that broke was burst shape. The **5 WCU / 3 RCU** the flip returned to the pool are headroom for `series`, not a budget to spend.
...
- **The honest inversion, re-scoped to the capacity still provisioned.** At list price `series`'s 14 WCU / 21 RCU would cost `14 × $0.000735 × 730 ≈ $7.51` plus `21 × $0.000147 × 730 ≈ $2.25` — **≈ $9.76/month** — against on-demand for the same work: `series` takes 4,850 of the cycle's 6,290 writes (the 4,800 forecast items plus 50 actuals), so `4,850 × 730 ≈ 3.54 M` write units at $0.705/M ≈ **$2.50**. Provisioned is **3.91×** the on-demand bill. ADR 0002's own pair — **≈ $11.30** against **≈ $2.88**, a **3.92×** — covered the whole pre-#156 pool, both tables' capacity together, and it computed them at us-east-1 rates on the same 730-hour month. The ADR is restated-with-a-correction here rather than amended, on the precedent the api stack's log-ingestion note sets below: its conclusion is the **ratio**, and the ratio survives both corrections at once. Changing Region moved every rate in the comparison by about the same 13% (12.8% to 13.2%; PITR, outside the ratio, moved +10%), and dropping `weather` out of both sides removed roughly a quarter of each — a ratio notices neither. What the corrections move is the absolute pair, and those are the numbers a reader would quote.
- **And on the one table where the free tier stopped winning, it did not lose on price.** A 7% duty cycle is exactly what on-demand pricing exists to serve; provisioned won on `series` and `weather` alike only because the tier is free, and ADR 0002 says so rather than pretending it is the better engineering answer. `weather` did not move because the arithmetic above changed — it still favours provisioned, and the flip gave up a free line for a ≈ $0.30 one. It moved because free capacity that cannot absorb a 25-item `BatchWriteItem` burst is not capacity the workload can use: the alarm fired live, 6 of 12 locations failed to publish, and no pool-fundable WCU number fixed the burst shape. That is the whole of the trade — **≈ $0.30/month to convert a cliff into a rate**, on the escape hatch `tables.tf` and ADR 0002's reversibility section both named in advance ([issue #156](https://github.com/TomBennett-Lloyd/cumulo/issues/156)).
- **If the always-free 25 GB turns out not to cover on-demand tables, the storage row above still holds.** ADR 0002 records this as the one thing it could not confirm from AWS's docs, and it noted the question was moot because the two tables holding essentially all the data were both provisioned — which, for `weather`, is no longer true. It stays moot for a different reason: `weather`'s share of the retained bytes is the small one. Its forecast rows are keyed `(locationId, FORECAST#T#<validTime>)`, and every cycle rewrites the same 48-hour window shifted one hour, so what is _retained_ is one row per location-hour rather than one per write, and the ~3.5 GB is dominated by `series`. Megabytes at $0.283/GB-month is cents either way.
...
- **Nothing here has an hourly rate**, which is the property to preserve, and #156 did not spend it — an on-demand table has no rate at all until something writes to it. That is the difference between this stack's ≈ $0.30 and a clock-driven charge: it is bounded by what the fleet does, and it goes to zero when the fleet does nothing. Per-request DynamoDB costs at this volume stay cents, the weather row being the largest of them at 0.3% of the ~$100 ceiling; a VPC, a NAT gateway, or a database instance would be the whole budget — the comparison ADR 0002 turned on.
...
| **DynamoDB writes** (`cumulo-series` via BatchWriteItem)           | ~2,880 items/hour of load on the **storage** stack's provisioned `cumulo-series` capacity — inside its free pool, so this stack adds load, not a line; the capacity row and its figures live in the [storage stack's table](#storage-stack)          | **$0.00/mo here** |
