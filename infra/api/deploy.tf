...
# .github/workflows/deploy-api.yml makes, on this stack's function, and nothing
# else. The second such grant on `cumulo-github-actions`, after
# infra/ingestion/deploy.tf, and written to the same shape deliberately —
# convention 8 in infra/README.md is that every grant the shared role holds
# lives in the stack that needed it.
#
...
data "aws_iam_policy_document" "github_actions_deploy" {
  ...
  statement {
    ...
    # account, including the ingestion function this stack has no business
    # touching.
...
resource "aws_iam_role_policy" "github_actions_deploy" {
  ...
  # applying the API a second time with `environment = "staging"` must add a
  # second inline policy rather than silently overwrite the first one and leave
  # the dev function undeployable.
  name = "cumulo-api-deploy-${var.environment}"
