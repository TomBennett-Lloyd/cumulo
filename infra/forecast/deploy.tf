...
# .github/workflows/deploy-forecast.yml makes, on this stack's function, and
# nothing else.
#
...
#
# The trust-policy question the ingestion stack's equivalent had to answer is
# already settled: `cumulo-github-actions` stopped being assumable from a PR
# context when the first grant was attached (infra/bootstrap/oidc.tf,
# infra/README.md convention 8). This is the third grant on that role, not the
# first, so it inherits that closure rather than forcing it.
...
#
# Unlike the DynamoDB and SQS names, this one is a real prerequisite rather than
# a runtime one: bootstrap is already applied before this stack (its state lives
...
data "aws_iam_policy_document" "github_actions_deploy" {
  ...
  # out from under Terraform — which matters more on this stack than on the
  # others, because this function's timeout is one half of a coupling with the
  # queue's visibility timeout (lambda.tf). And there is no
  # `lambda:UpdateEventSourceMapping`, so the batch size and the concurrency
  # cap are equally out of CI's reach. The code is the one field CI owns and
  # every other field stays a reviewable diff in this directory. There is no
  # `lambda:InvokeFunction` either: deploying is not running.
  statement {
    sid = "UpdateForecastFunctionCode"
    actions = [
    ...
    resources = [aws_lambda_function.forecast.arn]
  }
}
...
resource "aws_iam_role_policy" "github_actions_deploy" {
  ...
  # applying forecast a second time with `environment = "staging"` must add a
  # second inline policy rather than silently overwrite the first one and leave
  # the dev function undeployable.
  name = "cumulo-forecast-deploy-${var.environment}"
